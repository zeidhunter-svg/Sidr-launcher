...
Диапазон: `5f50f73..54afe4a` (+ этот коммит), **19** коммитов кода, ветка `launcher--7`. Спека:
...
```
`BUILD SUCCESSFUL`, **exit 0**, 558 actionable tasks (36 executed, 2 from cache, 520 up-to-date).
...
(подметание закрывает накопление, но не единичный экземпляр). `guarded` сужен до `catch(Exception)`,
