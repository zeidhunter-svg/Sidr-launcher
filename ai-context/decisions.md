...
конструирует второй стор), заметка для **A5**. И последнее, названное здесь потому, что до сих пор
жило только в KDoc стора: **стор пишет по лексически нормализованному пути**, а тот для `..`,
пересекающего симлинк, — **другой файл**, чем назвал вызывающий. Это согласовано с воротами по
построению (иначе один файл ключевался бы двумя способами), но перенацеливание происходит молча.
...
## 2026-09-03 — Этап 5 (A1′) — федерированный ToolRegistry: N источников за одной границей, provenance на поверхности, один risk→gate предикат

**Status: `CLOSED` (2026-09-10) — гейт зелёный **и** приёмка владельца на устройстве пройдена, каждое
остаточное ограничение названо и адресовано. `CLOSED` не означает нулевой долг. Раздел «Приёмка на
устройстве» в конце этого ADR — что именно наблюдалось; всё, что ниже до него, — запись на момент
написания ADR 2026-09-03.**

**Статус на 2026-09-03: `CODE-GREEN`. `DEVICE-ACCEPTED` — НЕ достигнут, и это говорилось прямо, а не
подразумевалось.**
Блок зарегистрировал и сделал **достижимыми из типизированной команды** два новых пользовательских
системных интента (`set_timer`, `open_system_settings`) — не просто добавил их в реестр, а провёл через
них живой goal без модели (`ToolMatchPlanner` + шаг 2b `RouteCommandUseCase`). Ни то, ни другое ни разу
не запускалось на телефоне. Это не вариант A0.5, где `DEVICE-ACCEPTED: неприменимо` было честным, потому
что тот блок не менял ничего на телефоне: этот блок меняет ровно то, что видит пользователь, а
собственный критерий успеха спеки №1 (§16) требует приёмки владельцем на устройстве. Статус — обычный
дефицит, не неприменимость, и `CLAUDE.md`/`current-status.md` (правка второго диспетча) обязаны нести
его так же прямо.

Диапазон: `3bcf0a5..23f9152`, **23 коммита** (пересчитано `git log --oneline 3bcf0a5~1..23f9152`, а не
взято из брифа диспетча, который называл 20). Ветка `launcher--7`. Спека:
[docs/superpowers/specs/2026-08-29-a1-federated-tool-registry-design.md](../docs/superpowers/specs/2026-08-29-a1-federated-tool-registry-design.md);
план: [docs/superpowers/plans/2026-08-29-a1-federated-tool-registry.md](../docs/superpowers/plans/2026-08-29-a1-federated-tool-registry.md);
полный леджер 13 задач, 21 руководящее решение (R1–R21), каждый раунд мутаций:
[.superpowers/sdd/2026-08-29-a1-federated-tool-registry/progress.md](../.superpowers/sdd/2026-08-29-a1-federated-tool-registry/progress.md).
...
`BUILD SUCCESSFUL in 1m 9s`, **exit 0**, **`557 actionable tasks: 557 executed`**. План текстуально
требует `--rerun`, которого нет как build-level флага в Gradle 9.5.0 (`--help` не знает его, только
`--rerun-tasks`) — это уже произвело один ложный зелёный внутри самого блока: задачный гейт задачи 3,
раунд 2, вернул `BUILD SUCCESSFUL` за 11 секунд, исполнив **ноль** из четырёх тестовых задач. Ruling R9
этого блока фиксирует замену, и этот закрывающий прогон использует `--rerun-tasks` и печатает строку
`N actionable tasks: N executed`, поэтому число ниже — наблюдение, а не вывод из инкрементальности
Gradle.

`core/ui` в этом блоке **не тронут**: `git diff --stat 3bcf0a5~1..23f9152 -- 'core/ui/**'` пуст, и
`git diff --name-only` по тому же диапазону не называет ни одного файла под `core/ui/`.
`:core:ui:verifyRoborazziDebug` поэтому в закрывающий гейт не включён — по тому же правилу, каким A0.5
включила его (блок его коснулся), здесь правило указывает на противоположный вывод.

**Числа прочитаны из JUnit XML под `build/test-results/`, не из консоли**; каждый файл проверен на
свежесть по времени модификации — все тринадцать наборов записаны между `08:58:40` и `08:59:13`,
внутри окна прогона `1m 09s`, закончившегося в `08:59:55`, то есть это результаты именно этого прогона,
не остатки прежнего:

| Модуль | База (2026-08-26, `CLAUDE.md`) | A1′ (2026-09-03, измерено) | Δ |
|---|---|---|---|
| `:domain:jvmTest` | 419 | 434 | +15 |
| `:app:testDebugUnitTest` | 33 | 42 | +9 |
| `:consumer:jvm:test` | 56 | 56 | 0 |
| `:data:repository:testDebugUnitTest` | 220 | 265 | +45 |
| `:core:android:testDebugUnitTest` | 12 | 12 | 0 |
| `:core:ui:testDebugUnitTest` | 128 | 128 | 0 |
| `:data:ai-cloud:testDebugUnitTest` | 32 | 32 | 0 |
| `:data:prayer:testDebugUnitTest` | 38 | 38 | 0 |
| `:feature:assistant:testDebugUnitTest` | 38 | 38 | 0 |
| `:feature:launcher:testDebugUnitTest` | 176 | 186 | +10 |
| `:feature:permission_education:testDebugUnitTest` | 15 | 15 | 0 |
| `:feature:prayer:testDebugUnitTest` | 19 | 19 | 0 |
| `:feature:settings:testDebugUnitTest` | 33 | 33 | 0 |
| **Итого** | **1219** | **1298** | **+79** |

Ноль падений, ноль ошибок, ноль пропущенных во всех тринадцати модулях. `assembleDebug` — exit 0 в том
же прогоне. Рост сосредоточен ровно там, где его предсказывает работа блока: `domain` (типы `ToolLevel`/
`ToolEffect`/`ToolWorker`/`ToolAdapter`/`ToolFederation`, `requiresConsent`, `ToolFederationTest`),
`data/repository` (второй источник, `ToolMatchPlanner`, локализованный словарь и его гварды, парити-тест
фейка), `feature/launcher` (поверхность provenance), `app` (три новых/переякоренных гварда). Модули, чьей
поверхности блок не касался (`core/android`, `core/ui`, `data/ai-cloud`, `data/prayer`,
`feature/assistant`, `feature/permission_education`, `feature/prayer`, `feature/settings`), — на нуле, что
и подтверждает отсутствие побочного блёст-радиуса.

### Что построено

Один `ToolRegistry`-порт, теперь федерирующий N адаптеров позади него, а не проецирующий один каталог:

```kotlin
@JvmInline value class ToolLevel(val value: String)      // открытый: "in_app", "system_intent", "sandbox", …
enum class ToolEffect { LOCAL, EXTERNAL }                 // закрытый на двух значениях — да/нет пересечения границы

interface ToolWorker { suspend fun invoke(invocation: ResolvedInvocation): ToolResult }
data class ToolAdapter(val level: ToolLevel, val registry: ToolRegistry, val worker: ToolWorker)
class ToolFederation(private val adapters: List<ToolAdapter>) { val registry: ToolRegistry; val executor: ToolExecutor }
```

- **`ToolFederation` — один объект, два лица.** `registry` и `executor` читают один и тот же список
  адаптеров; ни один провод Hilt не может развести их — это прямое лечение находки F2 ревью A0 (гейт
  читал `risk` из плана и `permissionGate` из живого реестра, два источника одного решения).
- **`ToolExecutorCallSiteGuardTest` переякорен, не ослаблен**: одна реализация `ToolExecutor`
  (`FederatedToolExecutor`), один вызов `toolExecutor.invoke(` в `AgentExecutor.kt`. Новый сиблинг,
  `ToolWorkerCallSiteGuardTest`, держит второй хоп: `ToolWorker` вызывается только из
  `FederatedToolExecutor`. Обе мутационно доказаны (задача 3: W1–W4 на новом гварде, E1–E2 на
  переякоренном; ни одна не всплыла ложно-зелёной ни на одном из пяти сканируемых корней).
- **Второй настоящий источник на Android**: `Tier0IntentToolSource` + `Tier0IntentToolWorker`, уровень
  `system_intent`, два инструмента, `set_timer` (arity 1, `SAFE`, `EXTERNAL`) и
  `open_system_settings` (arity 0, `SAFE`, `EXTERNAL`) — ни один новый Android-permission. Composition
  теперь измеримое свойство продукта: `:app`'s production-граф собирает `SystemIntentToolSource`
  (уровень `in_app`, спроецированный `DefaultActionCatalog`) и `Tier0IntentToolSource` в одну
  `ToolFederation`, и `DoctrineGuardTest` строит этот самый граф из тестового source set `:app`.
- **`requiresConsent(risk: ActionRiskLevel): Boolean`** в `domain/action` — один предикат вместо четырёх
  независимых спеллингов (`risk >= CONFIRM` в `AgentExecutor`, `risk != SAFE` в `RouteCommandUseCase`,
  `risk == SAFE` в `ResolutionPreferencePolicy`, `risk != SAFE` в
  `EvaluateLearnedChoiceDisplayStateUseCase`), с парити-тестом (все три сегодняшних уровня гейтятся как
  раньше) и «будильным» тестом (уровень ниже `CONFIRM` гейтится одинаково всеми четырьмя) —
  см. «Остаточные ограничения» о том, что именно этот второй тест доказывает и чего не доказывает.
- **Достижимость (§8.4 спеки, поправка, внесённая до кода)**: `ToolMatchPlanner` в `:data:repository` +
  шаг **2b** в `RouteCommandUseCase`, между существующей веткой агента (2) и проверкой `localOnlyMode`
  (3) — при промахе FastPath собирается `AgentGoal(text, GoalShape.Free(text))` и предлагается
  композитному планировщику (`CompositePlanner(listOf(TemplatePlanner(), toolMatchPlanner))`). Шаг
  ничего не знает про инструменты и падает открыто на `NoPlan`, поэтому при отсутствии совпадения
  поведение побайтово то же, что и раньше. `GoalShape` не получает нового значения — см. отдельный
  раздел ниже.
- **Provenance на поверхности (`DOC-ILM-2`)**: `AgentSessionPresentation.toolLabelFor`/
  `provenanceLabelFor`, отрендеренные в `AgentSessionSurface` и держимые поведенчески —
  `AgentSessionSurfaceProvenanceTest` (Robolectric + `createComposeRule`,
  `onNodeWithContentDescription("source SYSTEM INTENT · EXTERNAL")`) и `LauncherScreenAgentProvenanceTest`,
  которая рендерит настоящий `LauncherScreen` над настоящим `LauncherViewModel`, закрывая шов, где
  `toolProvenance = emptyMap()` раньше проходил бы молча. `en`/`ru`/`tr` — в том же коммите
  (`LocaleCompletenessGuardTest`); три ключа `launcher_tool_level_*` — Class A
  (`translatable="false"`), потому что каждый другой provenance-токен в проекте уже Class A
  (Ruling R18) — цифровая подпись владельца не тронута, т.к. Class A исключён из дайджеста.
- **`FakeToolRegistry.withA0Tools()` — парити-строка добавлена** (§9.4 спеки, задача 12): фикстура,
  которую CLAUDE.md называет «pinned to nothing», теперь сверяется по всем восьми полям
  `ToolDescriptor` с настоящим `SystemIntentToolSource(DefaultActionCatalog())`; собственная мутация
  ревьюера (`durability` `TRANSIENT → DURABLE`) красна.

### Семь развилок владельца (спека §2, решены до кода, 2026-08-29)

| # | Развилка | Решение |
|---|---|---|
| **F1** | Развилка A1: параллельный словарь инструментов vs. эволюция `ActionCatalog` на месте | **Параллельный словарь + федерация, идентичность C.** `ToolId`/`ToolDescriptor` остаются словарём агента; `ActionCatalog` становится одним адаптером из N — чем он фактически уже был. `ActionIds` не тронут. Идентичность C: id спроецированного инструмента **выводится** из `ActionId`, а не копируется вручную |
| **F2** | Объём блока: только границы, или границы + масса инструментов | **Границы + ровно один настоящий второй источник.** `B2`/`B4`-масса и `B3`-выбор становятся **`A1″`**, новым блоком с адресом в Master Plan §3.2/§3.6 |
| **F3** | Какой второй источник доказывает composition на телефоне | **`set_timer` + `open_system_settings`** (один Tier-0 адаптер, арности 1 и 0, ноль новых permission). Пересмотрено со `set_timer` + `dial` во втором проходе проектирования — см. §8.2 спеки |
| **F4** | Тип уровня инструмента в `commonMain` | **Открытый value class над `String`** (`ToolLevel`) — прямое применение измеренного A0.5 контраста: открытый value-тип стоит ноль правок ядра на добавление, закрытая сумма — четыре файла в трёх модулях |
| **F5** | Находка A0.5 «согласие срабатывает до связывания аргумента», адресованная `A1′/A4′` (слэш = не решено) | **A4′.** Порядок `validate → checkpointFor → resolve` — свойство рантайма, намеренно fail-safe; A1′ фиксирует адрес строкой, не решает |
| **F6** | `ArgType` — одна из четырёх словарных находок A0.5, адресованных A1′, теперь когда появился настоящий нестроковый аргумент (длительность таймера) | **Не расширен.** Отклонён с измеренной причиной, не отложен — второй флаг-значение всё ещё категориально недостаточен для JSON Schema MCP/AppFunctions; расширение стоило бы работы на входной границе модели (`ProposalValidator`) без приближения к конечному состоянию |
| **F7** | Какие инструменты, судя по конечному продукту, а не по объёму блока | Владелец делегировал F3 и F6 агенту на критерии «что лучше для Agentic OS в перспективе»; оба ответа и рассуждение — в §8/§10.4 спеки, обратимы по существу |

### Хостильное прочтение ADR A0.5 (§3 спеки) — шесть находок, ни одна не спрятана

`§HANDOFF` предписывает первым действием A1′ хостильное прочтение ADR A0.5, вопрос не «точно ли это?»,
а «говорит ли это то, что нужно A1′?», и запрещает переносить любое число оттуда неизмеренным. Оба
правила выполнены:

| § | Находка |
|---|---|
| §3.2 | Слово **provenance** встречается **ноль** раз в ADR/спеке/плане A0.5; `domain/tool` не содержит ни `provenance`, ни `tier`, ни `level`. Блок, построивший второй источник инструментов проекта, ни разу не задал вопрос «откуда взялся этот инструмент, и как это скажет трасса» — то есть весь предмет `DOC-ILM-2`. Крупнейшее молчание документа относительно этого блока |
| §3.3 | A0.5 — свидетельство **портируемости**, не **composition**. Два `ToolRegistry` существуют, но никогда не сосуществуют в одном реестре (Hilt связывает порт 1:1); блок не даёт наблюдения о коллизии `ToolId`, порядке источников или о том, что значит `all()` над N источниками — ровно предмете A1′. Следствие: «три id — ноль правок ядра» измеряет **добавление**, не **composition**, и число не переносимо как «дешёвое» |
| §3.4 | Ответ A0.5 на вопрос 3 Master Plan §3.1a («риск — свойство платформы или ядра?») — по существу материал `DOC-ADL-1`, под неверной этикеткой. ADR подшивает его как ответ о портируемости и не называет долг, который он закрывает |
| §3.5 | Найдено при проектировании: федерация реестра **подразумевает** федерацию **исполнителя**, и ни один governing-документ этого не говорит. `AgentExecutor` держит один `ToolExecutor`; без второго исполнителя второй источник даёт инструменты, которые планировщик видит, гейт пускает, а вызов молча падает в generic failure. Находка о **плане**, не об ADR — исправлена работой пункта 4 §4 спеки (федерация исполнителя строится вместе с реестром) |
| §3.6 | `§HANDOFF` утверждает «`args_json` на диске хранит `ArgSource`, а не значения» — **ложно как написано**: `AgentSessionMappers.kt:218` кодирует `ArgSource.Literal` **со значением**. Верно по замыслу (`FromStep` перевязывается против выживших наблюдений, а не проигрывает замороженное значение), ложно как прочитано — и первый черновик этой самой спеки вывел из буквального чтения неверное заключение (§8.2 спеки). Строка `§HANDOFF` должна быть исправлена в закрывающем проходе (второй диспетч) |
| §3.7 | `DOC-ADL-3` несёт долг A1′, не названный в Master Plan §3.2: клетка матрицы кончается «"наружу не уходит ничего" держится механически только для `domain/agent` + `domain/tool`… за их пределами это факт, а не проверка ← долг A1′ **вместе с федерацией источников**». §3.2 называет только `DOC-ILM-2` и `DOC-ADL-1` — блок на самом деле закрывает три доктринальных долга, третий — источником из матрицы, а не из определения блока |

### Поправка §8.4 — один общий рукав планировщика, не таксономия

Внесена в спеку **во время написания плана**, до кода — задокументированный change-control, не
находка задним числом. Спека, как первая написана, выбрала два инструмента и не спросила, **что их
планирует**. Единственный планировщик Android — `TemplatePlanner` над `GoalShape`, исчерпывающий:
`AppNotInstalled` даёт план запуск→магазин, `Free` даёт `NoPlan`, а `RouteCommandUseCase` строил только
`AppNotInstalled`. Оба Tier-0-инструмента были бы зарегистрированы и **непланируемы** — тот самый провал
«способность существует, но никогда не предлагается», который спека называет риском `B3`, наступивший
на два блока раньше срока.

Отклонённое лечение — добавить `GoalShape.SetTimer` и рукав планировщика на инструмент — работает и
дёшево один раз, но **линейно по инструментам**: двенадцать инструментов `A1″` потребовали бы двенадцать
форм и двенадцать рукавов, что в коде опровергает сам тезис блока — что двадцать первый инструмент
бесплатен, — и потратило бы `B1` (Master Plan §3.6, «не расти `GoalShape`»), чей заявленный режим отказа
— ровно рукописная таксономия.

Принятое лечение: **один общий рукав, `GoalShape` не получает нового значения.** `ToolMatchPlanner`
детерминированно сопоставляет текст с локализованным словарём инструментов; если ровно один
зарегистрированный инструмент совпадает и его обязательные аргументы заполнимы — одношаговый план,
иначе `NoPlan`. `TemplatePlanner` не тронут и по-прежнему владеет `AppNotInstalled`. `StepRationale`
тоже не получает нового значения: строка шага теперь читается по `ToolId`, а не предполагает запуск
(Ruling R5, задача 10) — что и держит A0 обратно совместимым (`launch_app`→
`launcher_agent_step_launch`, `play_store_search`→`launcher_agent_step_store`, побайтово).

### Мутационная таблица

Правило блока то же, что и в A0.5: **зелёный прогон нового гварда не доказывает ничего; доказывает
мутация.** Каждая — в одном shell-вызове с `trap … EXIT`, дерево проверено `git status --porcelain
--untracked-files=all` после.

| Раунд | Мутации | Итог |
|---|---|---|
| Задача 3 — `ToolWorkerCallSiteGuardTest` / переякоренный `ToolExecutorCallSiteGuardTest` | W1–W4 (в т.ч. W4 — исчезновение держателя, направление сжатия) + E1–E2 | все **RED** на именно названном ассерте; ни одного ложного зелёного на пяти сканируемых корнях |
| Задача 8, раунд 1 — `ToolVocabularyLocaleGuardTest` / `ToolVocabularyReachabilityTest` | A1 (четвёртая запись только с `en`), A2 (локальный ключ есть, набор форм пуст), A3 (ключ удалён), B1 (возврат затенённого триггера), B3 (мусорный триггер) | все **RED**; **B2 (пустая таблица записей) вернулась GREEN** — обе гварды вакуумны на пустом словаре, ни одна не проверяет непустоту → находка, раунд 2 |
| Задача 8, раунд 2 — floor в `guardedEntries()` | M1 (пустая таблица), M2 (удалён только `set_timer`, таблица не пуста), M3 (легитимная третья запись без дыр) | M1/M2 **RED** (называют недостающие id), M3 **GREEN** (floor не переходит в равенство — не топит легитимный рост); контрольная `tr`-дыра на необязательной третьей записи — ровно **1 из 5** RED, доказывая, что floor раздаёт полный список, а не отфильтрованный |
| Задача 11, раунд 1 — `DoctrineGuardTest` (черновая версия) | M2 (сетевой литерал), M3 (третий `ToolAdapter`), M4 (изъят `toolMatchPlanner` из композиции), M5 (комментарий называет id, реального рукава нет) | все **RED**; M4 — самая ценная (та самая форма, что уже один раз убила headline-способность блока). **F1 «no two registered tools share an id» и F2 (адаптер с нулём инструментов вакуумен) вернулись GREEN** → находки, раунд 2 |
| Задача 11, раунд 2 (после фикса F1/F2) | M1-повтор, M6-повтор (оба на общем accessor), M7 (изъят только `set_timer`), **M8 (легитимный третий инструмент)** | M1/M6/M7 **RED** с точным именованием недостающих id; **M8 — GREEN 5/5** — сторона над-пиннинга, которую раньше никто не проверял |
| Задача 12 — парити `FakeToolRegistry` | мутация ревьюера сверх заявленной имплементатором (`durability` `TRANSIENT → DURABLE`) | **RED** на именно названном парити-тесте |

Процессная находка внутри самих раундов (задача 11): второй раунд начался с **грязного** дерева —
незакоммиченный остаток самопроверки имплементатора через `cp`-восстановление вместо `git checkout --`
+ `trap`, отрапортовавшей «git diff HEAD чисто», хотя это было не так. Восстановлено прувером до старта.
Тот же A0.5-урок («самопроверка — не доказательство») пришёл в этот блок во второй раз.

### КРИТИЧЕСКАЯ находка (ревью задачи 10) — headline-способность блока была мертва в продакшне

Найдена ревьюером задачи 10, принадлежала задаче 9, не была закреплена ни за одной задачей плана.
`AgentSessionMappers.toSessionEntity` **бросал исключение** на `GoalShape.Free`, и тест **пиннил этот
бросок** — корректно, когда писался: планировщик A0 никогда не производил `Free`. Задача 9 изменила
предпосылку, скомпоновав `ToolMatchPlanner`, и ни одна задача не заметила разрыв. Живой путь: команда
таймера → шаг 2b строит `Free`-цель → план найден → `store.save` → маппер бросает → `save` заворачивает
это в `Failure` → `RouteCommandUseCase:147-150` молча пропускает → падение в модельный путь. **Ни сессии,
ни таймера, ни ошибки, и все 1285 тестов на тот момент зелёные** — потому что каждый слой был протестирован
изолированно.

Исправлено в `1c4a61e`: `GoalShape.Free` кодируется дискриминатором `"Free"`, `goal_shape_arg` хранит
собственный `text` формы (перестройка из `goal_text` была бы lossy-кодированием — два текста
типо-независимы, и `:consumer:jvm` уже строит их по-разному). **Схема не менялась**: diff `schemas/`
пуст, `identityHash` тот же `4f50e433566f387714cb09ecc5017737`, правка `RoomColumnNames.kt` (+8/-8) —
только KDoc. Ревьюер проверил фикс **независимо**, не по отчёту: в отдельном detached worktree откатил
только кодирующий рукав — оба теста `FreeTextGoalEndToEndTest` красны с точно тем же текстом; удалил
только декодирующий рукав `SHAPE_FREE` — снова красно. `FreeTextGoalEndToEndTest` реально пересекает
ранее непроверенный шов (настоящий Room, настоящий `CompositePlanner`, настоящая федерация, настоящий
`RouteCommandUseCase`; фейковы только FastPath/модель/флаги), и `assertEquals(0,
modelPlanner.planCallCount)` — то, что делает молчаливое проскальзывание наблюдаемым.

**Урок, самый дорогой в блоке: четыре слоя, каждый полностью протестирован изолированно, в сумме дали
мёртвую способность. Если у продуктового пути есть шов, который не пересекает ни один тест, этот путь
не проверен** — та же формулировка, что и ревью A0 записало про `restoreOnStart`, в другой форме.

### Класс fail-open — тронут в одном месте, не как класс; и шаг 2b изменил его цену

`RouteCommandUseCase:147-150` по-прежнему молча падает в модельный путь на **любом** не-`Success` от
стора — переполнение диска, битая строка, любой будущий сбой стора. Критическая находка выше была
**одним экземпляром** этого класса; сам класс не тронут этим блоком и остаётся за **A4′**.

**Новое здесь — не ветка, а её последствие для доктрины, и оно появилось вместе с 2b.** У ветки (2)
fail-open безвреден: она срабатывает, только когда FastPath **решил** (`Message(NoAppFound)`), поэтому
проваливание доходит до шага (4) и возвращает исходный FastPath-ответ — модель не вызывается, с
устройства не уходит ничего. Шаг **2b** срабатывает ровно на обратном условии
(`ruleOutcome.isUndecided()`), поэтому то же проваливание идёт дальше, к шагам (5)–(7), и **сырой текст
команды уходит в облачную модель** — на цели, которую зарегистрированный инструмент уже опознал
детерминированно. Цена сбоя стора перестала быть «пользователь не получил сессию» и стала «правило
*понимание — модели, исполнение — детерминированному слою* обходится **отказом хранилища**»: полный
диск или битая строка молча переводят локальную, работающую офлайн команду в сетевой исход — без
сессии, без ошибки, без трейса. Это **не** расширение `OutboundContextPolicy`: уходит ровно тот текст,
который ушёл бы, не совпади ни один инструмент. Но решение его отправить принимает ошибка диска, а не
правила маршрутизации, и именно этим — а не частотой отказа — поднят приоритет A4′.

### Остаточные ограничения (называть, не подразумевать отсутствующими — Этап 0.5)

- **Блок `CODE-GREEN`, НЕ `DEVICE-ACCEPTED`** (на момент написания ADR). Ничего из A1′ не запускалось
  на телефоне. Два пользовательски достижимых системных интента добавлены и сделаны достижимыми из
  типизированной команды; критерий успеха №1 спеки требует приёмки владельцем на устройстве.
  **Уточнено 2026-09-05:** владелец прогнал **один** пункт Части A чеклиста — он и нашёл дефект
  следующего пункта. Приёмкой это не стало ни в какой части. **СНЯТО 2026-09-10:** Части A и B
  пройдены владельцем на SM-A325F в локали `ru-RU`, блок — `CLOSED`; см. «Приёмка на устройстве» в
  конце ADR, включая то, чего приёмка не покрыла (`en`/`tr` на устройстве не запускались,
  `"sayaç ayarla"` носителем не судился).
- **Дополнение 2026-09-05, после закрытия этого ADR: первый прогон владельцем нашёл headline-инструмент
  блока мёртвым.** `AlarmClock.ACTION_SET_TIMER` требует `com.android.alarm.permission.SET_ALARM`,
  манифест его не объявлял, `ActivityTaskManager` отказывал каждому вызову — при том что восемь
  документов утверждали «ноль новых permission». То есть ложная посылка развилки F3 доехала до
  устройства и обошлась в блокирующий дефект. Увидеть это не мог ни один тест:
  `Tier0IntentToolWorkerTest` подсаживает фейковый `IntentLauncher`, поэтому настоящий `startActivity`
  не достигается, а у пропуска в манифесте юнит-тестовой сигнатуры нет вовсе. Permission объявлен,
  восемь документов исправлены, класс держится `ToolPermissionManifestGuardTest` (`:app`, `939d1d7`).
  Тот же прогон **фальсифицировал и обоснование `SAFE`** для `set_timer` («последнее действие — за
  пользователем»): при `EXTRA_SKIP_UI = false` часы Samsung открылись с уже идущим отсчётом. Оставлено
  владельцу, не переписано агентом; запись — в `CLAUDE.md`, Known debt.

- **Дополнение 2026-09-10 — владелец решил вопрос `SAFE`, и решён он в пользу «уровень остаётся,
  обоснование заменено».** `set_timer` остаётся `SAFE`; ни один риск-уровень и ни одна строка
  поведения не менялись — менялись только утверждения. Снятая формулировка: «ни один не обходит
  собственный UI ОС — последнее действие за пользователем», вместе с отсылкой к форме «предзаполнил,
  но не отправил» (Master Plan §3.6 `B4`). Она измерена ложной: `EXTRA_SKIP_UI` управляет тем,
  показывает ли отвечающее приложение свой UI, а не тем, действует ли оно. На чём `SAFE` стоит теперь
  — сказано, а не подразумевается: эффект **обратим одним касанием** (`Удалить` на только что
  запущенном таймере), **виден сразу** (часы открываются перед пользователем, ничего не происходит
  в фоне), **раскрыт** (`EXTERNAL` рисует строку provenance — единственный такой механизм у `SAFE`-шага,
  потому что гейт согласия для него не срабатывает) и **локален** (ни вызов, ни его аргумент не
  покидают устройство). `open_system_settings` — **другой** случай, и его нельзя складывать с этим:
  открытие экрана настроек не выполняет никакого действия, обращать нечего. Исправлено в
  `Tier0IntentToolSource`, `Tier0IntentToolWorker`, spec §8.1 и ячейке форка F3, чеклисте приёмки A1′
  и ячейке `B4` мастер-плана — последняя больше не подпирает эту фразу, и форму для `SENDTO`,
  календарного `INSERT`, `DIAL`, `geo:` блок A1″ обязан **измерить на устройстве**, а не унаследовать.
  Сама фальсификация сохранена намеренно: рейтинг, у которого заявленная причина оказалась ложной, —
  более полезная запись, чем тихо переписанная причина.

- **Дополнение 2026-09-10 — второй дефект того же прогона: закрытие агентной поверхности оставляло
  лаунчер в режиме поиска.** «закрыть» на результате плана (или «Отмена» на гейте согласия) убирала
  поверхность, но набранная команда оставалась в буфере — а «поиск перекрывает» есть чистая функция
  буфера (`LauncherCommandSession.liveResults`), поэтому шахада, строка даты и полоса намаза не
  возвращались, и лечил это только force-stop. `LauncherViewModel.dismissAgentSession` удалял сессию и
  больше ничего не делал (его же KDoc обещал два эффекта, а давал один), а путь отказа завершает сессию
  `Cancelled` — состоянием, которое поверхность рисует как ничто. **Происхождение — A0 (`5f04a1d`), не
  регрессия A1′**, но именно A1′ сделал дефект рядовым: до него агентную поверхность открывал только
  промах FastPath «нет такого приложения», после — любая распознанная командой инструментальная цель.
  Оба выхода теперь чистят буфер тем же вызовом, что и ✕ самого поля; держат три теста в
  `LauncherViewModelTest`, которые проверяют **видимое пользователю** состояние после выхода, а не факт
  удаления строки — прежняя проверка «сессия удалена» и есть то, что позволило дефекту уехать.
  Проверено на SM-A325F под управлением агента (обе формы плана — одношаговая инструментальная и
  двухшаговая A0); приёмкой это не является — владелец пере-прогнал обе части чеклиста на исправленной
  сборке в тот же день, см. «Приёмка на устройстве» в конце ADR.

  **Мутационный раунд этот гвард измерил — и потому это здесь, а не только указателем в `CLAUDE.md`:
  мутационные результаты по правилу маршрутизации §0 плана трека принадлежат ADR.** Основные
  направления держатся: снятое объявление в манифесте и неотображённое новое действие — оба RED с
  называнием недостающего; легитимный **полный** рост — GREEN, то есть над-пиннинга нет; на пустом
  скане 3 из 4 тестов всё ещё стреляют. Рукописная колонка `requiredPermissions` слаба ровно
  настолько, насколько объявлена в KDoc, и не сильнее: переписать её в «`ACTION_SET_TIMER` не требует
  ничего» оставляет все четыре теста зелёными, а приложение — в дофиксовом сломанном состоянии.
  Отдельной парой доказана и починка build-входа: без декларации манифеста как `Test`-входа в
  `app/build.gradle.kts` та же мутация возвращает `UP-TO-DATE` при exit 0 с живым дефектом, с ней —
  RED.

  **Два предела, измеренных тем же раундом; ни один из них не является живым дефектом сегодня** — оба
  отгруженных воркера строят интенты инлайн, манифест корректен. **(a)** `Intent(…)`, собранный в
  обычном объекте-хелпере, который зовёт воркер, невидим гварду **целиком**: файл попадает в
  сканируемое множество, только если он **и** объявляет `ToolWorker`, **и** сам содержит `Intent(`, а
  хелпер не `ToolWorker` — измерено инструментом `call_number`, чей воркер зовёт хелпер, собирающий
  `Intent.ACTION_CALL`, при отсутствующем в манифесте `android.permission.CALL_PHONE`: **зелено 4/4**,
  та же форма дефекта, ради которой гвард написан. **(b)** Гвард ключуется на **воркерах**, а не на
  зарегистрированных инструментах: тотальность срабатывает на `Intent(` в файле воркера, а не на
  `ToolDescriptor`, объявляемом `ToolRegistry`, поэтому инструмент, зарегистрированный в источнике без
  ветки воркера, невидим. Вопрос, на который класс отвечает, — «объявлен ли permission у каждого
  интента, выпускаемого сканируемым воркером», а не «может ли запуститься каждый зарегистрированный
  инструмент», хотя дефект 2026-09-05 был экземпляром именно второго. Это пределы **видимости**, и
  рискует ими автор адаптера №3, то есть **A1″**, всё содержание которого — новые Tier-0-интенты.
  Закрытие любого из двух — ключевание скана на том, что объявляет реестр: другой гвард, а не более
  широкая регулярка. Полная формулировка — в KDoc самого теста
  (`app/src/test/java/com/sidr/launcher/agent/ToolPermissionManifestGuardTest.kt`); адрес автору
  адаптера №3 — в `§HANDOFF` плана трека; указатель — в `CLAUDE.md`, Known debt.
- **Сдерживание исключений — по-воркерное; контракт `ToolWorker` не держится ничем механическим.**
  Финальное ревью ветки нашло КРИТИЧЕСКУЮ (находка 1, `356fe1a`): `ContextIntentLauncher.launch` звал
  ...
  активности для `AlarmClock.ACTION_SET_TIMER` (AOSP- или ROM-сборка, отключённый Deskclock)
  типизированная команда таймера доходила до одношагового `SAFE`-плана — гейт согласия её не
  останавливает — и убивала процесс домашнего экрана. Починено внутри `Tier0IntentToolWorker`:
  `ActivityNotFoundException`/`SecurityException` → `ToolResult.Failed(CommandFailure.Generic)`.
  Размещение подтверждено пере-ревью: воркер — последний слой, ещё способный выдать корректный
  типизированный результат, тогда как лончер, возвращающий `Unit`, отчитался бы об успехе для эффекта,
  которого не было, и трасса записала бы `Effected` — по `DOC-ILM-3` соврала бы. Трасса, которая
  врёт, хуже трассы, которая останавливается.

  **Остаточное — не сам баг, а то, чем держится контракт.** «Вызов инструмента всегда даёт
  `ToolResult`» — свойство `ToolWorker`, но сегодня оно держится соглашением плюс тремя реализациями,
  не механизмом. Сегодня оно не нарушено ни разу, и это проверено пофайлово:
  `Tier0IntentToolWorker` ловит две названные ошибки (набор — `AndroidActionExecutor`'s, сверенный по
  всем трём его `startActivity`-сайтам, у каждого оба `catch`); `SystemIntentToolWorker` своего `try`
  не имеет вовсе и наследует сдерживание от той же неизменной цепочки
  `ExecuteActionUseCase → IntentActionResolver → ActionExecutor`; `SandboxToolWorker` в `consumer/jvm`
  ловит широко — `catch (e: Exception)` с проброшенным `CancellationException` и намеренно не
  пойманным `Error`. Три реализации, три разные сети, ни одна из них не является **требованием**:
  **воркер четвёртого адаптера, который бросит, воспроизводит ровно этот краш**. Ни
  `ToolWorkerCallSiteGuardTest`, ни `ToolExecutorCallSiteGuardTest` его не увидят — они держат,
  **откуда** зовут, а не **что** вылетает обратно. Это дыра в том, что держится механически, а не
  живой краш: `Tier0ToolExecutionEndToEndTest` подсаживает бросающий `IntentLauncher` и проходит
  через весь движок, `Tier0IntentToolWorkerTest` держит обе ошибки на воркере.

  **Адресация: починка — A4′, обязательство до неё — A1″.** Чинится это в движке, а не в адаптере:
  `try` на единственной точке вызова в `AgentExecutor.perform` (или сдерживающая обёртка внутри
  `ToolFederation`), отображающий бросок в `ToolResult.Failed` и обычный `ToolObserved`. Аргумент,
  запретивший ловлю в `ContextIntentLauncher`, здесь **не действует и решения о размещении не
  отменяет**: там `launch` возвращает `Unit`, поэтому проглоченный отказ неотличим от успеха, — а на
  точке вызова тип результата уже `ToolResult`, поэтому пойманный бросок становится честным `Failed`,
  а не выдуманным `Effected`. Движок и модель исполнения — **A4′**, где уже лежат бюджет по стенным
  часам, staleness и класс fail-open. Но **встречается** эта дыра в **A1″**: он идёт следующим блоком,
  до A4′ (Master Plan §3, «Последовательность блоков»), и добавляет ~10 Tier-0-интентов плюс `LauncherApps`/ярлыки — то есть
  новых воркеров, каждый из которых заново решает этот вопрос сам. Поэтому A1″ обязан либо закрыть
  контракт механически, либо записать в своей спеке, что каждый новый воркер ловит сам, и на каком
  наборе ошибок. Не подразумевать это очевидным: автор адаптера №3 — ровно тот человек, который
  сегодня не узнает, что бросок из воркера убивает лаунчер.
- **Закрытие `DOC-ADL-1` опирается на единственную точку вызова, не на «будильный» тест (Ruling R4).**
  Написанный против `entries.first()` по обеим сторонам, `ConsentPolicyTest`'s wake-up-тест не может
  отличить `risk != entries.first()` от `risk >= CONFIRM`, пока существует ровно три уровня риска; он
  кусается в день, когда под `CONFIRM` вставится четвёртый. Он действительно убивает реальные мутации
  (`always true`, `== DANGEROUS`), и это показано, но закрытие правила — свойство единственного call
  site, не этого теста.
- **Ruling R7**: `core/testing/src/main/java` не сканируется ни одним из двух call-site гвардов —
  `ToolWorker`, объявленный в этом тестовом вспомогательном модуле, невидим
  `ToolWorkerCallSiteGuardTest`. Owner-level: расширение стоит ещё одного репо-широкого снимка на
  каждую тестовую задачу (семья D1/D4 из `§HANDOFF`).
- **`DoctrineGuardTest` не пиннит объявленный риск (мутация M9, задача 11).** `SAFE → CONFIRM` на уже
  зарегистрированном Tier-0-инструменте проходит гвард и весь `:app`-набор молча. Та же семья «pinned by
  nothing», что и `FakeToolRegistry` до этого блока. **Но незамеченным это не проходит ни для одного из
  четырёх сегодняшних инструментов** (уточнено финальным ревью ветки, находка 4 — занижать покрытие так
  же неверно, как завышать): `Tier0IntentToolSourceTest`'s `none { requiresConsent(it.risk) }` ловит
  мутацию для обоих Tier-0-инструментов, а парити-тест задачи 12 — для двух A0-инструментов; оба живут
  в `:data:repository`, чем и объясняется молчание `:app`-набора. Настоящая дыра — инструменты
  **будущего** адаптера: утверждение лежит в тесте источника, а не в гварде, поэтому новый источник
  приезжает без пина риска, если его автор не напишет свой.
- **Ruling R14 была наполовину неверна, и это доказал имплементатор мутацией.** `String.lowercase()` —
  `Locale.ROOT` с Kotlin 1.5; турецкая половина рассуждения контроллера («голый `.lowercase()`
  возвращает dotless-i баг») была ложной. Записано прямо: журнал решений, скрывающий свои ошибки, стоит
  меньше.
- **Правило `line()` ключуется на количестве аргументов.** Инструмент с двумя и более литеральными
  аргументами падает обратно на текст цели и воспроизводит дублирование, ради устранения которого
  правило существует. Верно для сегодняшних инструментов; названо как предел, не как общее свойство.
- **Шесть из тринадцати исходных триггеров плана не могли сработать никогда**, затенены FastPath
  (`LAUNCH_VERBS`/`INSTALL_VERBS` перехватывают их раньше), и турецкая колонка была полной на вид
  таблицей с **нулём** достижимых триггеров — найдено прогоном триггеров через настоящий matcher, теперь
  держится `ToolVocabularyReachabilityTest`.
- **`"sayaç ayarla"` нуждается в носителе языка** (owner-level, не решено агентом). Доказано достижимым
  и незатенённым, но `sayaç` читается как счётчик/измеритель, а не кухонный таймер. Ни один гвард не
  ловит «достижимо, но никто так не печатает».
- Матрица (`docs/governing/sidr-doctrine-matrix-v1.0.md:136`) для `DOC-HMA-2` по-прежнему говорит
  «уровней ещё нет» — формулировка, датируемая до этого блока. Этот блок ввёл `ToolLevel`, так что
  предложение неточно в деталях, хотя вывод строки («половина про уровни ← долг A1′») остаётся верным по
  существу — вопрос, останавливает ли **смену уровня** цикл, всё ещё открыт и адресован A4′ (спека §12).
  Не правится здесь: правка матрицы — задача второго диспетча.

### Критерии успеха (спека §16) — по одному, с доказательством

1. **НЕ ВЫПОЛНЕН на 2026-09-03; ВЫПОЛНЕН 2026-09-10.** «Два источника … на устройстве, принято
   владельцем» — механика была полностью на месте (федерация из двух адаптеров в проде,
   `Tier0IntentToolSource` достижим из шага 2b), но **устройство не участвовало**, и это был
   единственный критерий, державший блок на `CODE-GREEN`. Владелец прогнал Части A и B на SM-A325F
   2026-09-10 в локали `ru-RU` — обе пройдены, прогон нашёл и закрыл три вещи. Раздел «Приёмка на
   устройстве» ниже; локали `en`/`tr` на устройстве **не** запускались.
2. **ВЫПОЛНЕН.** `find()`/`all()` согласованы by construction — один объект `ToolFederation`, оба лица
   читают один и тот же список; `ToolFederationTest` (`all concatenates its sources in adapter order`,
   `find reaches a tool in any source`) говорит это явно.
3. **ВЫПОЛНЕН, но не с первой попытки — и это стоит записать.** Черновая версия `DoctrineGuardTest`
   читала `federation.registry.all()` уже **после** дедупликации федерации, поэтому дубликат никогда не
   достигал ассерта, который его должен был поймать (F1, задача 11) — на момент первого прохода критерий
   3 не выполнялся, хотя гейт был зелёным. Исправлено чтением того, что объявляют **адаптеры**, до
   дедупа (`productionAdapters().flatMap { it.registry.all() }`), мутационно доказано в обоих раундах.
4. **ВЫПОЛНЕН.** Каждый зарегистрированный инструмент несёт `level`/`effect` (оба — обязательные
   не-null поля без дефолта); `AgentSessionSurfaceProvenanceTest`/`LauncherScreenAgentProvenanceTest`
   краснеют, если убрать рендер; `DomainIdentifierLeakGuardTest` переуказан с `ToolTier` на `ToolLevel`
   (§6.5 спеки); сырой `ToolLevel.value` нигде не достигает UI sink — маппинг идёт через `sidrString`.
5. **ВЫПОЛНЕН формально, с named-оговоркой.** Все четыре сайта вызывают `requiresConsent`; парити-тест и
   wake-up-тест оба зелёные. Но оговорка, повторённая из «Остаточных ограничений», обязана сопровождать
   любое цитирование этого критерия: wake-up-тест не различает две гипотезы, пока существует три уровня
   — закрытие правила держится единственной точкой вызова, а не этим тестом (Ruling R4).
6. **ВЫПОЛНЕН.** `consumer/jvm` не менял `build.gradle.kts` в этом блоке (diff пуст) и по-прежнему
   зависит только от `:domain` (jvm-вариант) + `kotlinx-coroutines`/`kotlinx-serialization`;
   `ModuleIsolationTest`'s `no Android class is reachable from this module` (загрузка
   `android.content.Intent` через рефлексию терпит неудачу) — зелёный в этом прогоне. Полное разрешение
   зависимостей (`dependencyInsight`, как это сделала A0.5) в этом прогоне не переповторено — модуль не
   тронут структурно, и это то, на что опирается вывод.
7. **ВЫПОЛНЕН.** Гейт зелёный на 1298 тестов ≥ базовой линии 1219, 0 падений; каждый новый гвард
   мутационно доказан (мутационная таблица выше) — с оговоркой R7 про сканируемые корни, названной
   отдельно и не отменяющей факт мутационного доказательства того, что действительно просканировано.
8. **ВЫПОЛНЕН.** Три парити-теста (`localOnlyMode` / нет провайдера / offline) зелёные и сильнее
   заявленного — они же проверяют `planner.planCallCount == 0` и `agentStore.saved.isEmpty()`, не только
   равенство исходов; инвариантный тест «все три причины сразу — репортится только внешняя» остался
   зелёным без единой правки диффа (проверено ревьюером задачи 9 по факту нулевых удалений в файле).
9. **ВЫПОЛНЕН для трёх строк, `DOC-HMA-2` честно НЕ заявлен закрытым.** `DOC-ILM-2`, `DOC-ADL-1`,
   `DOC-ADL-3` несут реальные имена тестов в матрице, каждое проверено character-for-character против
   дерева в рамках этой самой сессии (`DoctrineGuardTest`, `AgentSessionSurfaceProvenanceTest`,
   `ConsentPolicyTest`, `AgentExecutorTest`, `RouteCommandUseCaseTest`, `AgentEgressSentinelGuardTest`,
   `AgentVocabularyGuardTest` — все существуют). `DOC-HMA-2` матрица оставляет как есть; спека §12
   называет его «половиной, а не закрытием» — вопрос смены уровня инструмента остаётся A4′.

**Итог по девяти критериям на 2026-09-03: 7 выполнены полностью, 1 (№5) выполнен формально с
обязательной оговоркой, 1 (№1) не выполнен.** Единственный невыполненный критерий — устройство — и он
один держал блок на `CODE-GREEN` вместо `CLOSED`. **После приёмки 2026-09-10: 8 выполнены, 1 (№5)
выполнен формально с той же обязательной оговоркой** — устройство её не видит и снять не могло.

### Чего блок не закрывает

Развилка A1 сама по себе (параллельный словарь vs. эволюция `ActionCatalog`) F1 разрешил в пользу
первого, но это была развилка **владельца**, а не пере-открытая агентом; блок её не переоткрывал.
`DOC-HMA-2` — половина, не закрытие (выше). Масса инструментов и выбор перед планировщиком (`B2`/`B3`/
`B4` сверх двух Tier-0) — адресованы `A1″`, новому блоку. `ObservedFact`-асимметрия §6.3 A0.5
по-прежнему не тронута — намеренно, по инструкции владельца от 2026-08-23, воспроизведённой без
изменений; диф, трогающий её, был бы откатом, а не результатом. Класс fail-open в
`RouteCommandUseCase:147-150` — не тронут этим блоком, адрес A4′. Ни `ActionIds`, ни
`OutboundContextPolicy.ALLOWED` не тронуты.

### Приёмка на устройстве — прогон владельца 2026-09-10 (SM-A325F / Android 13, `ru-RU`): блок становится `CLOSED`

Владелец выполнял пункты сам; агент готовил сборку, вёл прогон и записывал. Прогон `adb`/`uiautomator`
под управлением агента приёмкой не является (словарь Этапа 0.5), и ни один пункт ниже на такой прогон
не опирается: там, где доказательство инструментальное, это сказано прямо и оно ничего не закрывает.

**Состояние устройства.** SM-A325F, Android 13, локаль `ru-RU` — та же, что у Task 15 (A0) и у
пере-проверки 2026-08-23, ровно затем, чтобы сравнивать с уже принятым. Debug-сборка ставилась
**поверх** существующей, без удаления: схема осталась `user_version = 4`, блок её не менял, миграция
не запускалась и не должна была. Первый заход шёл на сборке `b1c9424` (в ней уже была починка
`SET_ALARM` от 2026-09-05), финальный — на `1ef158d`, то есть после двух починок, произведённых между
заходами самой приёмкой (`43751f8`, `f228d10`), и решения владельца по `SAFE` (`1ef158d`).

**Часть A — то, на что владелец обязан посмотреть глазами. Пройдена целиком.**

1. **A1, таймер.** «поставь таймер на 5 минут» → открылись настоящие часы Samsung, таймер на
   5 минут; на агентной поверхности — одношаговый выполненный план, строка шага «Поставить таймер на
   5 минут», под ней `SYSTEM INTENT · EXTERNAL`. **Таймер уже шёл**, когда часы открылись — та самая
   фальсификация заявленного обоснования `SAFE`, которую владелец решил в тот же день (дополнение
   2026-09-10 выше).
2. **A2, настройки.** «системные настройки» → открылось настоящее приложение «Настройки» Android, не
   собственный экран Sidr; строка шага «Открыть настройки Android», та же строка provenance.
   **Негативная проверка** «открой настройки системы» повела себя как задумано: `LAUNCH_VERBS`
   FastPath'а забирает «открой» первым, и владелец увидел двухшаговый план A0 (запуск → предложить
   магазин), а не экран настроек. Это задокументированное в KDoc `ToolVocabulary` затенение, не дефект.
3. **A3, provenance на поверхности A0.** Присутствует и корректна. Владелец увидел также
   `АГЕНТ · LAUNCH_APP · PLAY_STORE_SEARCH` — проверено, что это **не** привнесено A1′: строка была
   уже в A0 (`a7f4755`).
4. **A4, шесть новых строк.** Прочитаны и **приняты как есть**, поправок нет. Ни одна из трёх Class B
   не потребовала правки, подпись локалей не переподписывалась.

**Часть B — прогоны на отсутствие регресса. Пройдена целиком на финальном прогоне.** Двухшаговый план
A0 доехал до конца через шаг магазина (`market://search` выпущен); глаголы FastPath, которых A1′ не
касался, работают как прежде; Assistant/BYOK стримит нормально; отмена и `force-stop` оставляют
лаунчер здоровым; три таблицы `agent_*` в покое — `0/0/0`. Два пункта Части B **упали на первом
заходе, и оба падения были ошибкой измерения, а не дефектом продукта** — находка 2.

#### Три находки прогона — все починены, каждая записана по своей причине

**1. `set_timer` не мог запуститься ни разу** (найдено 2026-09-05, первым пунктом Части A; полностью —
в дополнении 2026-09-05 выше). `AlarmClock.ACTION_SET_TIMER` требует
`com.android.alarm.permission.SET_ALARM`; манифест его не объявлял, восемь документов утверждали
«ноль новых permission». Починено `939d1d7`, класс держится `ToolPermissionManifestGuardTest`.
Headline-инструмент блока был мёртв на **каждом** устройстве, и увидеть это мог только телефон.

**2. Два «дефекта», о которых прогон отчитался в середине, были ошибкой измерения контроллера — и это
записывается, а не заминается.** Оба симптома («сессия замерла сразу после `PlanCreated`» и
«терминальные сессии не удаляются») — **один** дефект чтения на двух разных границах чекпоинта.
Чеклист сам предписывал `adb exec-out run-as … cat databases/sidr_history.db` (строка, которую агент
вписал туда же коммитом `2fe6fbf`), а Room держит базу в **WAL**: закоммиченная запись живёт в
`sidr_history.db-wal` и попадает в основной файл только на чекпоинте. На устройстве рядом с основным
файлом в 81 КБ лежал `-wal` в 428 КБ. Измерено, а не выведено: в один и тот же момент чтение только
основного файла давало `agent_session=1`, чтение пары `.db + -wal` — `0`. **Дефекта продукта не было;
каскадное удаление работает** — переснято на устройстве корректным чтением по всем пяти точкам
(одношаговый инструмент → пусто; двухшаговый план A0 → `AwaitingConsent`, курсор 1, 2 шага, 5 событий
трассы; «Отмена» → пусто; `force-stop` → пусто; перезапуск → пусто). И строка 2026-08-22, из-за которой
это выглядело регрессией, — та же недетерминированность границы чекпоинта, а не изменение поведения.
**То же неверное чтение врёт и в обратную сторону**: оно может показать все три `agent_*` пустыми,
пока строка с `agent_session.goal_text` — сырым текстом команды пользователя — лежит на диске. Это
**ложно-зелёное на гарантии приватности**, и именно поэтому починка — механизм, а не абзац прозы:
`tools/device/pull-agent-db.sh` (тянет пару, гоняет `integrity_check`, печатает три таблицы и счётчики
покоя) и `DeviceDatabaseReadGuardTest` (`:app`, три теста, скан фенсед-блоков под
`docs/superpowers/plans`: блок, читающий базу, обязан тянуть и `-wal` либо звать скрипт; скан
не-вакуумен; ни один блок не зовёт `sqlite3` на устройстве — его там нет). Проверено тремя мутациями
и восстановлением: возвращённый усечённый блок → RED, и задача **переисполняется** (объявление
`inputs.dir` несущее, базовый повтор был `UP-TO-DATE`); добавленный блок с `run-as … sqlite3` → RED;
`PLANS_DIR` в несуществующую папку → все три RED, включая тест на не-вакуумность, а не тихий пас;
восстановление → GREEN 3/3.
Названный предел гварда: он видит только `docs/superpowers/plans`; чеклист в `docs/governing/` или
`ai-context/` ему невидим, расширение скана обязано идти вместе с расширением `inputs.dir` в том же
коммите. Починено `43751f8`. **Самая дорогая ошибка этой приёмки принадлежит не продукту, а методу
измерения, которым её вели** — прогон едва не был записан как проваленный на работающем движке, и
запись, которая это скрывает, стоила бы меньше самой приёмки. Прецедент того же класса в репозитории
третий: `gradlew | tail` (2026-07-13), build-level `--rerun` (A1′), теперь `cat` мимо WAL.

**3. Выход с агентной поверхности оставлял лаунчер в режиме поиска** (полностью — в дополнении
2026-09-10 выше). «закрыть» на результате плана и «Отмена» на гейте согласия убирали поверхность, но
не чистили буфер команды, а «поиск перекрывает» есть чистая функция буфера — шахада, строка даты и
полоса намаза не возвращались, лечил только force-stop. Происхождение — A0 (`5f04a1d`), но рядовым
дефект сделал именно A1′: до него агентную поверхность открывал только промах FastPath «нет такого
приложения», после — любая распознанная инструментальная команда. Починено `f228d10`, держат три теста
в `LauncherViewModelTest`, проверяющие **видимое пользователю** состояние после выхода.

#### Два решения владельца, принятые в ходе прогона

- **`set_timer` остаётся `SAFE`; заменено обоснование** (`1ef158d`). `EXTRA_SKIP_UI` управляет тем,
  показывает ли отвечающее приложение свой UI, а не тем, действует ли оно, — поэтому снятая фраза «ни
  один не обходит собственный UI ОС, последнее действие за пользователем» измерена ложной. `SAFE`
  стоит теперь на другом и это сказано, а не подразумевается: эффект обратим одним касанием, виден
  сразу, раскрыт строкой provenance и локален. Ни один risk-уровень и ни одна строка поведения не
  менялись — менялись утверждения. Сама фальсификация сохранена: рейтинг с ложной заявленной причиной
  — более полезная запись, чем тихо переписанная причина.
- **Блок закрыт: `CLOSED`** — гейт зелёный **и** приёмка владельца на устройстве пройдена, при том что
  каждое остаточное ограничение названо и адресовано, а не подразумевается отсутствующим. `CLOSED`
  здесь **не** означает нулевой долг (прецедент DS-6B): всё из раздела «Остаточные ограничения» выше
  переходит дальше **решением владельца**, а не потому, что перестало существовать.

#### Чего эта приёмка НЕ покрыла — говорится прямо, а не опускается

- **Прогнана одна локаль — `ru`.** `en` и `tr` на телефоне не запускались вовсе. Строки обеих локалей
  отгружены в том же коммите и проверены гвардами полноты, но это не то же самое, что «работает на
  устройстве».
- **`"sayaç ayarla"` (турецкий триггер `set_timer`) носителем языка НЕ судился.** Чеклист (§A1)
  требует называть это явно, а не пропускать молча: носителя на прогоне не было, «предположительно
  нормально» здесь не является допустимым ответом. Триггер отгружен и доказан достижимым и
  незатенённым (`ToolVocabularyReachabilityTest`) — это механический факт, а не суждение о том, что
  так кто-то печатает. Вопрос остаётся **владельческим и открытым**, адрес — A1″; если вердикт
  окажется «убрать», это правка кода (`ToolVocabulary`'s `tr`-набор), а не документов.
- **Часть C ничего не закрывает по построению** (федерация как один объект, отказ дублирующегося
  `ToolId`, `SAFE → CONFIRM` на уже зарегистрированном инструменте, слепое пятно `core/testing`,
  fail-open класс `RouteCommandUseCase`) — это агентское/тестовое свидетельство рядом с приёмкой, а не
  её часть.
- **`DOC-HMA-2` этим блоком НЕ закрыт.** Уровни инструментов введены, но останавливает ли цикл именно
  **смена** уровня — по-прежнему не построено; долг A4′.
- **Наблюдение о покое базы, которое стоит записать честно:** после удаления сессии сырой
  `goal_text` был **прочитан из образа файла на диске** — SQLite освобождает страницы, но не обнуляет
  их без `secure_delete`, а что здесь ставит сборка Android, не устанавливалось. На SQL-уровне
  гарантия «в покое ничего нет» верна и переснята пятью точками выше; файл лежит в приватном хранилище
  приложения и достижим только через `run-as` на debug-сборке. Менять journal mode или `secure_delete`
  на принятой схеме 4 — решение владельца; агент не трогал. Адрес: владелец / A4′.

**Масштаб этого утверждения — один блок, одно устройство, одна локаль.** Спека §16, критерий 1
(«два источника … на устройстве, принято владельцем») **ВЫПОЛНЕН 2026-09-10**; итог по девяти
критериям становится «8 выполнены, 1 (№5) выполнен формально с обязательной оговоркой». Оговорка №5
(`DOC-ADL-1` держится единственной точкой вызова, а не wake-up-тестом) приёмкой не снята и снята быть
не могла: устройство её не видит.

---

## 2026-09-19 — Этап 5.5 (A1″) — масса инструментов и отбор: пять действующих инструментов, четвёртый адаптер, и паттерн «свидетельство, описывающее не то, что от него думают»

**Status: `CODE-GREEN`. `DEVICE-ACCEPTED` — НЕ достигнут, и это говорится прямо, а не
подразумевается.** **Исправлено 2026-09-20 (задача 6 плана фазы 3b): формулировка, стоявшая здесь до
этой правки — «за весь блок на телефоне не запускалось НИЧЕГО из того, что он построил», — ШИРЕ
правды, а занижённый дефицит относится к тому же классу ошибки, что и завышенный успех.** Проверено по
git, а не по прозе: `ab2d063` (2026-09-16) — **смоук-раунд задачи 13b**, санкционированный владельцем и
**не являющийся приёмкой**, — выполнялся на SM-A325F и получил показание с устройства по **отгруженному
коду фаз 1–2**: S1 — `ShortcutToolSource` / `ShortcutCatalog` / `AndroidShortcutQuery`, сведённые так,
как их сводит `AgentProvidesModule`, дали **216 дескрипторов из 65 пакетов**, без коллизий
идентификаторов, все `app_shortcut` / `EXTERNAL` / `SAFE` / `TRANSIENT`, ноль пустых ярлыков; S3/S4 —
`ToolSelector` над этим реальным реестром из 216 инструментов на восьми текстах: авторский бьёт
динамический (**≈1 мс**), динамические кандидаты **≈64–69 мс**, три задокументированных отказа; S5 —
команда, проведённая через собственный UI лаунчера в `ru-RU`: ярлык исполнился через
`LauncherApps.startShortcut`, строка шага несла сторонний ярлык, провенанс отрисовался как
**`APP SHORTCUT · EXTERNAL`**, а «Закрыть» очистила буфер команды и вернула тело домашнего экрана;
S7/S8 — с отобранной ролью HOME набор инструментов отказал закрыто, без падения, авторские инструменты
целы. **Точная формулировка, принятая во всех четырёх документах: фазы 1–2 получили показание с
устройства в смоук-раунде, который не является приёмкой; фаза 3a — весь действующий набор, карточка
`CONFIRM`, адаптер `launcher_memory` и каждый триггер `tr`/`en` — на телефоне не запускалась вовсе, и
фаза 3b тоже.** Телефон в блоке был и в раунде измерений Task 13/13b (ниже), но тот измерял **премиссы
Android**, а не продукт. Ни один из пяти действующих инструментов, ни карточка согласия первого
`CONFIRM`-инструмента, ни один турецкий триггер на устройстве не наблюдались. Это обычный
дефицит, **не неприменимость** (в отличие от A0.5, где `DEVICE-ACCEPTED: неприменимо` было честным, потому
что тот блок не менял ничего на телефоне): этот блок меняет ровно то, что видит пользователь. Чеклист
приёмки написан и лежит в
...
`CLOSED` = гейт + приёмка владельца + названные остаточные ограничения, и до приёмки слово `CLOSED`
здесь не употребляется. `CLOSED` при этом не означает нулевой долг (прецедент DS-6B).

**Блок закрывается НЕ полностью исполненным, и это решение, а не умолчание.** Спека §7.6 разделила
работу на 3a (действующий набор) и 3b (одиннадцать навигирующих инструментов) и явно разрешила закрытие
с границы 3a: «на её границе продукт связен и пол уже выполнен». Пол выполнен с запасом — **пять**
действующих инструментов против четырёх. **3b не начат** и переносится следующим заездом; §16 критерий 1
(«≥15 инструментов из ≥3 уровней, достижимых на телефоне владельца, приёмка владельцем») этим блоком
поэтому **не выполнен** — см. §«Что блок не сделал» в конце.

> **Обновлено 2026-09-20.** Фаза 3b **выполнена** отдельным заездом — одиннадцать навигирующих
> инструментов, гейт 1439, `CODE-GREEN`; см. раздел «Фаза 3b — одиннадцать навигирующих инструментов»
> в конце этого ADR. Абзац выше описывает состояние на 2026-09-19 и сохранён как запись решения
> закрыть блок с границы 3a. Критерий 1 §16 остаётся **не выполненным**, но теперь ровно по одной
> причине: приёмки владельца на устройстве не было.

Диапазон: `a6d9130..ea9c528`, **76 коммитов** (`git log --oneline 265fd67..ea9c528 | wc -l` = 77, из
которых `265fd67` — закрывающий коммит A1′, в блок не входит; первый коммит блока — `a6d9130`, спека).
Ветка `launcher--7`. Спека:
...
планы: [фазы 0–2](../docs/superpowers/plans/2026-09-12-a1-second-tool-mass-and-selection.md) (13 задач)
и [фаза 3a](../docs/superpowers/plans/2026-09-18-a1-phase3a-acting-tools.md) (ревизия 2.2, 13 задач);
измерения на устройстве:
[docs/superpowers/plans/2026-09-12-a1-device-measurements.md](../docs/superpowers/plans/2026-09-12-a1-device-measurements.md).
Полный леджер обеих половин — 2735 строк, 44 руководящих решения R14-1…R14-44, каждый раунд мутаций:
`.superpowers/sdd/2026-09-12-a1-second-tool-mass-and-selection/progress.md` (**git-ignored**, умирает от
`git clean -fdx`; всё долговечное из него перенесено сюда и в `§HANDOFF` плана трека — это и есть
причина, по которой настоящий ADR длиннее обычного).
...
`BUILD SUCCESSFUL in 1m 15s`, **exit 0**, **`557 actionable tasks: 557 executed`**, 0 вхождений
`FAILED` в логе. Прогон сделан с **очищенного** `build/test-results` — по причине, которая сама является
находкой блока (R14-44 ниже). `--rerun-tasks`, а не `--rerun`: последний не существует как build-level
флаг в Gradle 9.5.0 и молча возвращает всё `UP-TO-DATE`, печатая `BUILD SUCCESSFUL` (один ложный зелёный
он уже произвёл внутри A1′).

**Числа прочитаны из JUnit XML под `build/test-results/`, не из консоли:**

| Модуль | База A1′ (1314, 2026-09-10) | A1″ (2026-09-19, измерено) | Δ |
|---|---|---|---|
| `:domain:jvmTest` | 434 | **440** | +6 |
| `:consumer:jvm:test` | 56 | 56 | 0 |
| `:data:repository` | 271 | **374** | +103 |
| `:feature:launcher` | 189 | **194** | +5 |
| `:app` | 49 | **59** | +10 |
| `:core:ui` | 128 | 128 | 0 |
| `:core:android` | 12 | 12 | 0 |
| `:data:ai-cloud` | 32 | 32 | 0 |
| `:data:prayer` | 38 | 38 | 0 |
| `:feature:assistant` | 38 | 38 | 0 |
| `:feature:permission_education` | 15 | 15 | 0 |
| `:feature:prayer` | 19 | 19 | 0 |
| `:feature:settings` | 33 | 33 | 0 |
| **Итого** | **1314** | **1438** | **+124** |

Ноль падений, ноль ошибок во всех тринадцати модулях. Рост сосредоточен там, где его предсказывает
работа блока: `:data:repository` (третий и четвёртый адаптеры, `ToolSelector`, двухслотовый словарь,
пять новых инструментов и их достижимость), `:app` (два новых гуарда, ключуемых по реестру),
`:domain` (сдерживание броска в движке, `ToolFederation` без кэша), `:feature:launcher` (карточка
согласия, называющая пакет, и два выхода с агентной поверхности). Модули, чьей поверхности блок не
касался, — на нуле, что и подтверждает отсутствие бокового радиуса.

**Четыре границы фаз, и на каждой предсказанная декомпозиция сверялась с измерением до прогона:**
`d391350` = 1321 (фаза 0) → Task 4 → 1323 → `09d4322` = **1357** (фаза 1; 1323 + 32 + 2, сошлось) →
`c05fd7a` = 1369 → `1db2cd8` = **1375** (фаза 2; 1369 + 5 + 1, сошлось) → `ea9c528` = **1438**
(фаза 3a; предсказанный пол был >1425, вывод держался цифра в цифру: 1425 + 12 + 1). Сверять итог с
разложением, а не только итог — это проверка, которую сумма одна сделать не может: она не отличает
«добавили два» от «добавили два и потеряли два».

`core/ui` блоком не тронут (`git diff --stat a6d9130~1..ea9c528 -- 'core/ui/**'` пуст), поэтому
`:core:ui:verifyRoborazziDebug` в закрывающий гейт не включён — по тому же правилу, по которому A0.5
его включила.

### Что построено

#### Фаза 0 — преконды движка (`d391350`, 1321 тест)

Три обязательства, которые §HANDOFF A1′ адресовал именно этому блоку, закрыты **до** первого нового
воркера — потому что блок собирался отгрузить их десятками, а не по одному:

1. **Бросок из любого воркера сдерживается в ДВИЖКЕ, на единственной точке вызова** (`5fba6de`,
   спека §8.1). `AgentExecutor.perform`'s `toolExecutor.invoke` получил `try`: пойманный бросок
   становится `ToolResult.Failed` и обычным `ToolObserved`, `CancellationException` перебрасывается.
   Это **прямо закрывает остаточное ограничение (8) блока A1′** («сдерживание исключений попёрсорно,
   держится соглашением плюс три реализации, а не механизмом»). Аргумент, запретивший ловить в
   `ContextIntentLauncher` (`Unit`-возвращающий launcher не отличит проглоченный отказ от успеха),
   здесь не применим: тип результата точки вызова уже `ToolResult`, поэтому пойманный бросок
   становится честным `Failed`, а не выдуманным `Effected`. Попёрсорные ловушки остались на месте —
   они дают *конкретные* отказы, а это пол под ними.
2. **Гуард разрешений, ключуемый по РЕЕСТРУ, а не по файлам воркеров** (`62f5c89`, спека §8.2) —
   `ToolRegistryPermissionGuardTest`. Оба измеренных слепых пятна предшественника
   (`Intent(…)`, построенный во вспомогательной функции; инструмент, зарегистрированный источником без
   ветки воркера) покраснели под мутацией. Старый `ToolPermissionManifestGuardTest` не удалён: они
   отвечают на разные вопросы, и §13 спеки записывает, на какие именно.
3. **Объявленный риск закреплён над продакшн-федерацией** (`4ce4edd`, спека §8.3) — `declaredRisk` в
   `DoctrineGuardTest`, с двумя раздельно нагруженными ассерциями (тотальность и дрейф), сработавшими
   в мутационном раунде независимо. Это **прямо закрывает остаточное ограничение (4) блока A1′**
   («`DoctrineGuardTest` не пинит объявленный риск; будущий адаптер приезжает без пина»).

Плюс `ToolPermissionCatalog` (`f48d05a`) — одна продакшн-карта `ToolId → разрешения`, где отсутствующая
строка **не означает согласия**, и фильтр «источник не рекламирует то, чего не может выполнить»
(`fc5d1e3`), и перепроверка преконда **непосредственно перед диспатчем**, чтобы отказ не записался как
`Effected` (`3d5f255`). Вместе это §7.7: детекция отказа — **преконд, а не лучший сигнал ошибки**.

#### Фаза 1 — движущийся набор инструментов и третий адаптер (`09d4322`, 1357 тестов)

- **`ToolFederation` перестала кэшировать при конструировании** (`cfe4985`, спека §4): обе грани —
  `registry` и `executor` — выводятся на каждом вызове, поэтому источник, чей набор меняется при живом
  процессе, виден обеим. Исчезнувший инструмент даёт `Failed`, а не падение и не ложный успех.
- **`ShortcutToolSource` — третий адаптер, уровень `app_shortcut`** (`f8d0051`, спека §5 / `B2`):
  один агентный инструмент на каждый ярлык Android-приложения, **имя которого — данные**, а не
  литерал. Первый источник, чья доступность — рантайм-**роль** (`android.app.role.HOME`), а не
  разрешение: на устройстве, где Sidr не домашнее приложение, каталог пуст и источник рекламирует
  **ничего** — требуемая деградация, а не отказ. `SAFE` с названным пределом: свойство «мы знаем, что
  он делает» из решения по `set_timer` сюда **не переносится**, потому что контракт `ShortcutInfo` не
  запрещает приложению отгрузить ярлык, действующий немедленно. Измерено на SM-A325F: 205 инструментов
  из 65 пакетов.

#### Фаза 2 — отбор (`1db2cd8`, 1375 тестов)

**`ToolSelector`** (`de3274a`, `92e8704`, спека §6 / `B3`) — один командный текст, максимум один
инструмент, над реестром, который теперь частично **сторонний и динамический**. Политика целиком в
порядке правил, и каждое правило стоит там из-за режима отказа, а не из предпочтения:
(1) авторский словарь бьёт динамический **безоговорочно**, включая случай, когда сам авторский словарь
неоднозначен — два наших вхождения, претендующих на один текст, обязаны отказать, а не дать ярлыку
приложения разрешить ничью; (2) динамический кандидат обязан назвать своё приложение; (3) равные
кандидаты **отказывают** — `singleOrNull` на *любых* двух попаданиях, а не только на равноспецифичных;
(4) каждый токен команды должен быть покрыт (Task 10b, одобрено владельцем сверх плана) — иначе
«system settings for my car» запускало бы ярлык.

**`B3`'s обоснование измерено ЛОЖНЫМ, и Master Plan исправлен по собственному change-control**
(`171f4b3`, `c13ff69`). Строка утверждала «двести дескрипторов в промпт не кладутся — требование, а не
оптимизация». Измерено: 200 дескрипторов ≈ 713 токенов, ≈ 6 035 пессимистично, ≈ 8 290 с гипотетическим
описанием. Отбор нужен по **другой** причине — §6.1: неоднозначность и сторонние имена, срабатывающие с
эффектом. Правильная вещь, неправильное обоснование; исходное утверждение оставлено видимым, измерение
поставлено рядом (прецедент `B4`). **Тем же решением добавлена строка `B10`** — мост от реестра
инструментов к модели, которым не владел ни один блок; её пять барьеров названы, из них два найдены
только при перепроверке против кода, а не против собственного резюме: у модельного предложения
**инструмента** нет валидатора (`ProposalValidator` валидирует только против `ActionIds`), и `ArgType`
при одном значении — отказ A1′ был явно обусловлен отсутствием потребителя «модель над инструментами»,
а `B10` и есть этот потребитель. Рендер инструментов в облачную модель отправил бы список установленных
приложений и имена ярлыков с устройства ⇒ расширение `OutboundContextPolicy` ⇒ ADR + посаженный
сентинел по change-control §5.

#### Раунд измерений на устройстве — Task 13/13b (`eb5979a..d3d9663`)

Каждая клетка таблицы кандидатов §7.2 заполнена **измерением на SM-A325F**, ни одна не унаследована
(§16 критерий 8). Среди закрытых строк: 27 — `SET_ALARM` требуется, и отказ **громкий**
(`SecurityException`); 16/28/32 — отказ на стороне **отвечающего** невидим вызывающему
(`startActivity` возвращается нормально и не бросает, что бы деинсталлятор ни сделал); 33 — отказ
**заранее знаем** изнутри процесса через `checkSelfPermission`; 13/29 — `ACTION_SET_ALARM` создаёт
будильник **уже включённым**. Владелец в этом раунде перевернул роль `HOME` и снял запрет на
переустановку приложения; отдельным коммитом (`d3d9663`) исполнен fail-closed путь и телефон возвращён
в исходное состояние. **Результат раунда для спеки:** из тринадцати настольных строк выжили
двенадцать, из которых **действовала одна** — и это стало причиной переработки набора кандидатов.

**Владелец отверг вывод предыдущей сессии** («раз действует одна — нужно понижать пол»), и отказ верен
по существу: §7.1 исключает **свободный текст**, а не **действие**. Одиннадцать навигирующих строк —
то, что случайно перечислила настольная таблица, а не то, что вынуждает правило. Рамка, тоже
владельческая: «сидр — это не просто ланчер, а агентная ос» — кандидат судится по тому, что агент
**должен уметь на этом устройстве под детерминированными воротами**, и никогда по тому, что принято
уметь ланчерам.

#### Фаза 3a — действующий набор (`ea9c528`, 1438 тестов), 13 задач

**Пять действующих инструментов при поле в четыре.** Все зарегистрированы, все достижимы из
набранного текста в трёх локалях, все проходят через детерминированные ворота:

| Инструмент | Уровень | Аргументы | Риск | Задача |
|---|---|---|---|---|
| `set_alarm` | `system_intent` | время | `SAFE` | 5 (`abc35b5`) |
| `uninstall_app` | `system_intent` | пакет + (необяз.) ярлык | **`CONFIRM`** | 7 (`9c823f1`) |
| `set_app_alias` | `launcher_memory` | **два** слота (app, phrase) | `SAFE` | 9 (`c9ed89b`) |
| `forget_app_alias` | `launcher_memory` | phrase | `SAFE` | 9 |
| `forget_learned_choice` | `launcher_memory` | phrase | `SAFE` | 9 |

- **`AppTargetResolver`** (Task 6, `82d87c8`) — имя разрешается в **один** пакет или в ничто, и
  никогда не угадывает.
- **Разрешение имени поднято ВЫШЕ гейта согласия** (Task 6b, `2136555`) — форк **F5**, который A0.5
  записала как адрес, а не как решение. До этого карточка согласия для первого `CONFIRM`-инструмента
  трека могла назвать только то, что пользователь **набрал**, потому что пакета, который будет удалён,
  в тот момент ещё не существовало. Закрыто для детерминированно разрешимых аргументов — не в общем
  виде, и план это говорит.
- **`uninstall_app` — первый `CONFIRM`-инструмент трека**, и он **отказывается удалять сам себя**.
  Шесть владельческих условий (2026-09-18) отгружены как тесты, а не как предложения; манифестная
  строка `android.permission.REQUEST_DELETE_PACKAGES` — решение владельца от 2026-09-18, `normal`,
  install-time, держится гуардом.
- **Четвёртый адаптер — `launcher_memory`** (Task 9): агент меняет **собственную память** по явной
  команде. Три тонких адаптера над контрактами, которые уже отгружены (`SaveAliasUseCase`,
  `DeleteAliasUseCase`, `DeleteLearnedChoiceUseCase`). Все три `TRANSIENT`, все `LOCAL`: ничто не
  покидает хранилище ланчера, и эффект отменяется из Настроек. Правка `:domain` — **одна константа**
  `ToolLevels.LAUNCHER_MEMORY` на **открытом** value-классе над `String`, поэтому правило CLAUDE.md
  «закрытая сумма в `commonMain` — не правка одного файла» не применяется, и измеренная цена A0.5
  (четыре файла в трёх модулях за одно значение `GoalShape`) не платится. `:domain:jvmTest` остался на
  439, что это и подтверждает.
- **Двухслотовая форма в словаре** (Task 8, `5a7a339`) — единственная примыкающая к движку
  способность, которой потребовал действующий набор: `<A> <инфикс> <B> <суффикс>`. §7.4 (запрет
  свободнотекстовых аргументов) к `set_app_alias` не применяется **механически, а не в порядке
  исключения**: назначение фразы, `SaveAliasUseCase`, само прогоняет её через тот же
  `CommandNormalizer.normalize`, поэтому нормализация здесь **безпотерьна по построению**.
- **Триггеры в трёх локалях** (Task 10, `486b4b6`) — одна правка одного файла, `DEFAULT_ENTRIES`;
  проверка на столкновение с FastPath **выполнена**, а не предположена (реализатор прочитал
  `LAUNCH_VERBS`/`INSTALL_VERBS`/`SEARCH_VERBS`/`SETTINGS_KEYWORDS`/`SIMPLE_COMMANDS` напрямую), и
  `"alarm kur"` исключён именно потому, что `INSTALL_VERBS.suffixByLocale["tr"]` содержит `"kur"`.
- **Карточка согласия называет ПАКЕТ, а не то, что набрали** (Task 11, `1f314cc`) — владельческое
  условие 2 держится сквозь: `PlanStep.line` читает `app`/`app_label` **по имени** через выделенную
  двухаргументную ветку для `uninstall_app`, а не через путь `singleOrNull()`, который два значения
  отрисовать не может. Что пользователь читает перед нажатием «Подтвердить»:
  `ru` — «Следующее действие Sidr: Удалить Telegram (org.telegram.messenger). Ничего не выполнится,
  пока вы не подтвердите.»
- **Сквозной шов** (Task 12, `ea9c528`) — `AgentActingSeamTest`, 648 строк, 12 тестов, **ноль правок
  продакшн-кода** (`git show --name-only | grep src/main` пуст), что и должно быть у шовного теста.

**Класс B не тронут, и это проверено на логике гейта, а не на файле.** Task 11 добавил ключ в
`strings_locked.xml` — это Класс B **файл**; `app/build.gradle.kts:186` относит ключ к Классу B, только
если `translatable != "false"`, а новый ключ `launcher_tool_level_launcher_memory` несёт ровно этот
атрибут. `git diff 486b4b6 1f314cc -- feature/launcher/src/main/res/` не содержит ни строки
`OWNER-REVIEWED`, ни `sha256`. **Подпись не трогалась, ничего не переподписывалось, sha256 агентом не
считался.** Владельцу переподписывать **нечего**.

### ПАТТЕРН БЛОКА — семь продемонстрированных случаев одной болезни

**Это самое ценное, что произвёл блок, и оно ценнее любого из пяти инструментов.** Единая формулировка:
**свидетельство, которое описывает не то, что от него думают.** Семь случаев, две причины, один симптом
и одна цена — зелёный тест, стоящий вместо непроверенного свойства.

**Четыре раза усиление рантайм-инварианта сделало охранявший его гуард НЕОПРОВЕРЖИМЫМ.** Причина в том,
что никто не предупреждает: улучшение продукта молча лишает тест способности падать.

- **R14-15.** Fail-closed фильтр Task 2 (хорошее свойство) сделал состояние «зарегистрирован без строки
  каталога» **недостижимым** для этого источника, поэтому мутация 3 задачи 4 могла быть только зелёной —
  **пустая мутация внутри протокола, который существует ровно для того, чтобы ловить пустые гуарды.**
- **R14-28.** То же самое, увиденное во второй раз и доведённое до следствия: гуард «у каждого
  зарегистрированного инструмента есть строка разрешений» больше **не может** упасть для
  `Tier0IntentToolSource`. Конкретно и близко: Task 5 добавляет `set_alarm`, забывает строку ⇒ инструмент
  удержан ⇒ никогда не зарегистрирован ⇒ **все четыре гуарда зелёные** ⇒ **инструмент отгружается
  невидимым**. Это дефект 2026-09-05 («зарегистрирован, но отказывает»), вывернутый наизнанку в «молча
  никогда не зарегистрирован».
- **R14-31.** Пол, закрывающий это **процедурно**, а не механически, — и он отгружен с **честно
  названным третьим случаем, остающимся зелёным**: строка удалена для отгруженного инструмента → RED;
  дескриптор добавлен, список пола расширен, строка забыта → RED; дескриптор добавлен, **список пола не
  расширен**, строка забыта → **по-прежнему GREEN**. Закрыть третий случай механически означает выставить
  наружу *объявленные* дескрипторы источника — продакшн-API ради теста, чего раунд не санкционировал и
  что не санкционировалось вслепую. Все три случая записаны в KDoc теста, и обязательство «расширить
  список пола И `REQUIRED_TOOL_IDS` в том же коммите» переносилось в каждый диспатч явно, а не как
  надежда.
- **R14-36.** Первый `CONFIRM`-инструмент **опроверг** ассерцию `none { requiresConsent(it.risk) }` — ту
  самую, на которую KDoc **в другом файле** (`DoctrineGuardTest`) ссылался как на пин рисков Tier-0.
  То есть инструмент, ради защиты которого пин и существует, не мог быть добавлен без правки теста,
  утверждающего, что такого инструмента не бывает, — а очевидная починка (удалить ассерцию) **снимает
  пин**. Заменено на покарточный риск-маппинг, который сильнее в обе стороны: он ловит и падение
  инструмента **до** `SAFE`, что ближе к отказу 2026-09-05, чем исходная формулировка.

**Три раза гуард НИКОГДА не мог упасть, и вскрылось это только мутацией.** Другая причина, тот же симптом.

- **R14-40** (Task 8). Тест «инфикс матчится как целое слово, а не внутри слова» с единственным входом
  «называй какао какао». Реализатор мутировал `twoSlotMatch` в голый `indexOf(form)` без окружающих
  пробелов — **сборка осталась зелёной**: «как» стоит на индексе 0 в «какао», поэтому левый слот пуст
  и у правильной, и у сломанной реализации, и обе отказывают **по совпадению**. Добавлен
  различающий случай `"называй текак телега"`, где подстрочный баг даёт непустые `app="те"` /
  `phrase="телега"`, то есть **неверное совпадение**, а не отказ; та же мутация → RED. Продакшн-код был
  верен всё время — пустым было доказательство.
- **R14-43, два случая** (Task 11), и это первый раз, когда **автор теста сам поймал себя**.
  (а) Новый тест на алиас **проходил против неисправленного кода**: текст цели «назови telegram телега»
  буквально содержал обе подстроки, поэтому дофиксовый откат на текст цели удовлетворял ассерцию по
  неверной причине. (б) Мутация `app_label` в `val label = pkg` **всё равно проходила**, потому что
  фикстурный пакет `org.telegram.messenger` **содержит** ярлык `telegram` как подстроку. Пакет заменён
  на `com.example.msg`, не делящий подстрок ни с одним ожидаемым значением, — RED получен.
  **Обобщение, и оно не про эти два теста:** ассерция рендеринга, чьи ожидаемые значения делят подстроки
  друг с другом или с текстом отката, **не может отличить верный вывод от нескольких неверных**.
  **Реалистичные фикстуры здесь самые опасные, потому что настоящее имя пакета обычно содержит
  настоящий ярлык.**

**R14-44 — та же болезнь на уровне ГЕЙТА, а не отдельного теста.** При первом чтении XML после сдачи
Task 12 `:app` показывал **8 тестов, 2 падения**, а итог читался **1387**, не 1438. С продуктом всё было
в порядке: это был остаток **мутации M8** (`uninstall_app` → `SAFE`, прогнанной фильтрованно по `:app`
**после** финального гейта и корректно откаченной в исходниках). Исходники были чисты; несвежим было
**свидетельство**. Режим отказа в том, что **остаток мутационного прогона неотличим от красного гейта**,
и указывает он в **обе** стороны: контроллер либо не верит верному отчёту, либо — хуже — более поздний
читатель принимает оставшийся зелёный от **до**-мутационного прогона за доказательство **после**-мутационного
дерева. **Лекарство процедурное и дешёвое, и оно теперь правило блока: последнее действие мутационной
задачи — перепрогон гейта с очищенного `build/test-results`, чтобы то, что лежит на диске, описывало то,
что закоммичено.** Настоящий закрывающий гейт этого ADR сделан именно так.

**Родственное, стоящее рядом, но НЕ то же самое.** Поверхность гуард-списков для «добавить инструмент»
выросла **ПЯТЬ → ШЕСТЬ → СЕМЬ → ВОСЕМЬ** за две сессии (R14-32, R14-33, R14-36, R14-41), и верного числа
не называет **ни один** KDoc, потому что число не стоит на месте: «пять» R14-32 и «шесть» R14-33 были
недосчётами уже в момент, когда их писали. R14-33 при этом обязан храниться **отдельно** от остальных:
шестой список ключуется на текст интент-константы, а не на `ToolId`, и он покраснел **громко**, в тот
момент, когда скомпилировалось новое построение `Intent`, — это не тихий пол, и путать его с ними нельзя.
**R14-41 — про МЕТОД, и это его единственная ценность: поверхность гуардов для «добавить инструмент»
нельзя перечислить заранее по документу; её можно только обнаружить прогоном гейта.** Восьмой список
(`ToolWorkerCallSiteGuardTest`'s пол «объявленные держатели `ToolWorker` — ровно известные пять») был
назван никем — ни брифом, ни диспатчем контроллера, перечислившим семь поверхностей, — и найден тем, что
регистрация четвёртого адаптера сделала их шесть и гуард упал **поведенчески**. **Настоящее закрытие
этого — гуард, ключуемый по объявлениям РЕЕСТРА (спека §8.2), и он явно не этой фазы:** фаза 0 построила
его для *разрешений*, но список держателей воркеров и списки-полы по `ToolId` по-прежнему рукописны. Тот,
кто диспатчит адаптер №5, обязан сказать это вслух, а не передать очередной список, который окажется
коротким.

### Хорошая половина, и её нужно назвать, а не только провалы

- **M1a (Task 12) — лучшая мутация блока.** Переключение `uninstall_app` в `SAFE` с **сохранением**
  `DURABLE` покраснело **не потому, что цикл не остановился, а потому что он остановился ПО ДРУГОЙ
  ПРИЧИНЕ**: `expected:<[ConsentRequested(index=0, reason=RISK_LEVEL)]> but was:<[…reason=DURABLE_EFFECT)]>`.
  То есть Critical C1 фазы держится **поведенчески** и различает **ПОЧЕМУ** сработали ворота, а не
  только **ЧТО** они сработали. Девять мутаций, девять RED на предполагаемой ассерции; M7 — **контроль**
  для сентинел-теста, единственный, кто ловит пустой рендер, без которого сентинельная половина
  проходила бы впустую.
- **Честная промежуточная сдача Task 9, на которой держится вся верификационная дисциплина блока.**
  Вынужденный отчитаться до окончания верификационного прогона, реализатор сказал прямо: «у меня ещё нет
  exit code и JUnit XML, и я их не выдумываю», перечислил, что именно не подтверждено, и **рекомендовал
  контроллеру не считать сдачу завершением**. Затем доделал и отчитался настоящими числами. Это
  поведение, без которого всё остальное в этом ADR было бы недоказуемо, и оно стоит рядом с провалами.
- **Дисциплина фикстур в Task 12, сразу после R14-43:** `zephyr` → `com.quill.notepad`, `kestrel` →
  `net.ownward.shell`, алиас `marmoset`, неразрешимый `pangolin` — ни одно значение не делит подстроку
  ни с другим, ни с текстом цели, и `OWN_PACKAGE` намеренно **не** `com.sidr.launcher`, чтобы тест не
  спутал «ключуется по внедрённому имени» с «узнаёт литерал».
- **Отсутствие Step-2 RED названо, а не сфабриковано.** У Task 12 его не было — задача не добавляет
  продакшн-символа, в существование которого красный тест мог бы его загнать. Реализатор сказал это
  прямо и подставил девять мутаций. Это **более сильное** свидетельство: первый красный доказывает лишь
  отсутствие, а каждая мутация называет, **какая** ассерция несущая.
- **Холодное ревью нашло то, чего не нашёл автор** — и это измерение, а не принцип: два Critical'а,
  изменившие продукт, были невидимы и тестам плана, **и** собственной перепроверке контроллера от
  2026-09-18, которая нашла два других настоящих дефекта и пропустила эти.

### Остаточные ограничения — названы, а не подразумеваются отсутствующими

- **R14-37 — зеркало R14-35, и НИ ОДНО из двух направлений не обобщено.** `ToolMatchPlanner` разрешает
  аргумент по имени `app` **независимо от `required`**. Будущий дескриптор с **необязательным** `app`
  (например `share_to(app, text)`) получит `raw=""` → `resolve("")` → `null` → **`NoPlan` НА ВСЕ ЦЕЛИ
  НАВСЕГДА**, при полностью зелёном прогоне. R14-35 закрыл противоположное направление
  (`required = true`) **точечным пином для `uninstall_app`**. Это собственный режим отказа блока —
  инструмент зарегистрирован и мёртв — повторяющийся для следующего автора. Долг `ArgType` на уровне
  планировщика; адрес — A4′ / A1″.
- **R14-38 — отказ «не удаляю сам себя» срабатывает ПОСЛЕ согласия.** `AppTargetResolver` не исключает
  собственный пакет, поэтому «удали sidr» разрешается в наш пакет, рисует `CONFIRM`-карточку **с его
  именем**, и только затем воркер возвращает `Failed(Generic)`. Пользователь соглашается на то, что
  произойти не может, и получает общий отказ. **Спеке это соответствует** — условие 4 стоит именно там —
  и закреплено тестом как текущее поведение, с KDoc, говорящим, что перенос отказа выше гейта есть
  решение **владельца**. Собственный аргумент Task 6b («отказать до того, как появится план, чтобы
  карточка не рисовалась для цели, по которой мы не будем действовать») применим здесь в равной мере.
- **R14-39 — доктринальный, и это самый важный из трёх.** Task 6b добавил **НОВЫЙ источник `NoPlan`**, а
  `NoPlan` на шаге 2b **проваливается к облачной модели**. Раньше `NoPlan` на 2b значил «инструмент не
  совпал»; теперь он значит ещё и «инструмент **СОВПАЛ**, а цель не разрешилась». То есть «удали
  приложение &lt;неизвестное&gt;» отправляет сырой текст команды в облако **на цели, которую
  зарегистрированный инструмент разобрал детерминированно**. Это **НЕ** расширение
  `OutboundContextPolicy` (тот же текст, который ушёл бы модельным путём, если бы не совпало ничего), но
  **решение отправить принимается неразрешённым именем, а не правилами маршрутизации** — то есть
  deterministic-first обходится по той же схеме, которую `CLAUDE.md` уже несёт как остаточное
  ограничение (3) блока A1′, теперь со **вторым** источником. Тест это закрепляет, и его **первая**
  ассерция — несущая: она доказывает, что словарь **действительно претендовал** на текст
  (`ToolVocabulary().match(…)?.id == UNINSTALL_APP`), прежде чем показать, что провал доходит до
  планировщика с сырой командой. Без неё тест закреплял бы обычный промах, а не доктринальный остаток.
  **Адрес — A4′.**
- **ШЕСТЬ `tr`-триггеров, НИ ОДИН не осуждён носителем языка** — `alarm ayarla`, `uygulamasını kaldır`,
  `kaldır`, `için`/`kullan`, `adını unut`, `için seçimi unut`, плюс унаследованный от A1′
  `sayaç ayarla` (итого семь незасуженных форм). До этого блока незасуженным был **один** триггер.
  **R14-42** фиксирует решение контроллера отгрузить `için`/`kullan` с **названным** ограничением, а не
  блокировать фазу вопросом, на который ни агент, ни гуард ответить не могут. Лингвистическая находка
  реальна и она — полезная часть: буквальные турецкие передачи «как» (`olarak` / `diye`) — **послелоги
  при ВТОРОМ аргументе**, стоящие непосредственно перед глаголом, поэтому они **не** занимают слот
  **между** аргументами и в фиксированный порядок `<A> <инфикс> <B> <суффикс>` не ложатся; `için` («для»)
  следует за управляемым существительным, то есть стоит ровно в позиции A, и форма ложится буквально.
  Наименее уверенная форма — **`için`/`kullan`** («понятно, но регистр скорее инструктивный, чем вполне
  идиоматичный», оценка реализатора). Цена, если неверно: `tr`-триггер, достижимый, незатенённый,
  доказанный мутацией и **никем не набираемый** — ровно форма `"sayaç ayarla"`, которую `CLAUDE.md` уже
  несёт нерешённой. **Это НУМЕРОВАННЫЙ ПУНКТ чеклиста приёмки, а не фраза в ADR** (прецедент §15
  чеклиста A1′: «предположительно нормально» — неприемлемый ответ на эту строку).
- **Двухслотовая форма ограничена порядком.** `<A> <инфикс> <B> <суффикс>` — это форма, а не общий
  механизм: язык, чья связка не занимает позицию между аргументами, в неё не ложится, что и обнаружено
  на турецком выше. Названо как предел, а не как свойство.
- **Ограничение (5) `CLAUDE.md` (правило строки шага ключуется на КОЛИЧЕСТВЕ аргументов) сужено, но не
  снято.** Task 11 дал `uninstall_app` собственную двухаргументную ветку, читающую аргументы **по
  имени**; правило по количеству осталось для всех остальных, и инструмент с двумя и более литералами
  без своей ветки по-прежнему откатывается на текст цели.
- **Долги, этой фазой не тронутые и уже записанные в `CLAUDE.md`:** **M3** — fail-closed на
  отсутствующей строке нетестируем на обоих потребителях (`ToolPermissionCatalog` — финальный класс);
  **M4** — два рукописных утверждения одного разрешительного факта с разными ключами, и блок наблюдал
  **инстанс** этого в дикой природе (R14-33); **D1–D11** — без изменений.
- **Открытый владельческий вопрос из фазы 2:** нужен ли **полокальный набор стоп-слов**. Без него любое
  служебное слово убивает динамическое совпадение («new chat in whatsapp», «новое сообщение в телеграм»).
  Назван в KDoc `ToolSelector`.
- **Раунд правок 2, находки #2, #4, #5 — ЗАКРЫТЫ в этой сессии** (только документация, поведение не
  менялось; гейт перепрогнан, 1438/0/0 без изменений, что и ожидается от комментариев). **#2** —
  `DoctrineGuardTest` ссылался на удалённую Task 7 ассерцию `none { requiresConsent(it.risk) }` и говорил
  «два Tier-0 инструмента», где их четыре; исправлено вместе с двумя близнецами в `CLAUDE.md` и
  `ai-context/current-status.md`, и на их место поставлена запись о том, **почему** ассерция исчезла
  (R14-36), — потому что это тот KDoc, по которому аудитор ищет, где риск на самом деле закреплён.
  **#4** — утверждение, что `EXTRA_SKIP_UI` не даёт шага согласия «в любом случае», сужено до
  измеренного: строки 13/29 мерили только `false`, и это то значение, которое воркер посылает.
  **#5** — четыре из шести мест «преконд, не детекция» получили предел, которого требует §7.7 (закрыта
  ровно **одна** причина отказа — отсутствующее разрешение; политика устройства, рабочий профиль и
  неудаляемый пакет остаются **ненаблюдаемыми**). Ни одно место не говорило «detected» утвердительно —
  жёсткий запрет был цел, это была мягкая половина правила.

### Критерии успеха спеки §16 — по одному, честно

| № | Критерий | Итог |
|---|---|---|
| 1 | ≥15 инструментов из ≥3 уровней, достижимых **на телефоне владельца**, приёмка владельцем | **НЕ ВЫПОЛНЕН** — 3b не начат, приёмки не было. Зарегистрировано девять авторских инструментов на четырёх уровнях плюс динамическая семья `app_shortcut`; достижимы из текста семь (пять действующих + `set_timer`/`open_system_settings`) |
| 2 | Движущийся набор виден **обеими** гранями федерации, исчезнувший инструмент даёт `Failed` | выполнен (`cfe4985`, фаза 1) |
| 3 | Бросок воркера сдержан **в движке**, на единственной точке вызова, с намеренно бросающим фейком | выполнен (`5fba6de`) — закрывает остаток (8) блока A1′ |
| 4 | Гуард разрешений ключуется по объявлениям реестра; оба слепых пятна красны под мутацией | выполнен (`62f5c89`) |
| 5 | Объявленный риск закреплён над продакшн-федерацией | выполнен (`4ce4edd`) — закрывает остаток (4) блока A1′ |
| 6 | Отбор отказывает на неоднозначности; авторский триггер не затеняется сторонним ярлыком | выполнен (`de3274a`, `92e8704`, `b35e489`) |
| 7 | `GoalShape` при двух значениях; `ActionIds`, `OutboundContextPolicy.ALLOWED`, `ObservedFact`, `CommandFailure`, `ArgType` не тронуты | выполнен — ни один замороженный тип не в диффе; `:domain` вырос на **одну** константу открытого value-класса |
| 8 | Каждая клетка §7.2 заполнена измерением на SM-A325F, ни одно Android-утверждение не унаследовано | выполнен (Task 13/13b) |
| 9 | Гейт ≥1314, 0 падений, из прогона, напечатавшего `N actionable tasks: N executed`; каждый новый гуард доказан мутацией в отдельном месте | выполнен — **1438**, `557 actionable tasks: 557 executed` |
| 10 | `B2`, `B3`, `B4` отвечены как включено / отвергнуто-с-причиной / переадресовано, и ADR несёт те же три ответа | выполнен: **`B2` включено** (адаптер `app_shortcut`, фаза 1); **`B3` включено, но его обоснование измерено ЛОЖНЫМ** и Master Plan исправлен по change-control — отбор нужен из-за неоднозначности, а не из-за размера промпта; **`B4` переадресовано** — «prefilled but not sent» измерено ложным на устройстве (`set_timer` и `set_alarm` создают эффект сразу), `SAFE` стоит на четырёх других свойствах |

**Итог по девяти построимым критериям: восемь выполнены, один (№1) выполнен частично и явно не закрыт,
потому что 3b не начат и приёмки не было.**

> **Обновлено 2026-09-20.** Строка 1 таблицы выше читается «3b не начат» — это было верно на
> 2026-09-19 и больше не верно: фаза 3b выполнена (раздел в конце ADR), зарегистрировано **двадцать**
> авторских инструментов на трёх уровнях плюс динамическая семья `app_shortcut`, достижимы из текста
> восемнадцать. Критерий 1 по-прежнему **НЕ ВЫПОЛНЕН** — исключительно из-за отсутствия приёмки
> владельца. Заодно поправка к самой клетке: авторские инструменты живут на **трёх** уровнях
> (`in_app`, `system_intent`, `launcher_memory`) — четвёртый уровень, `app_shortcut`, несёт только
> динамическую семью и ни одного авторского инструмента. Числа в самой таблице не переписаны
> намеренно: она — запись состояния на дату закрытия 3a.

### Что блок не сделал — сказано, а не подразумевается

1. **Ничего не запускалось на телефоне** из того, что блок построил. Статус `CODE-GREEN`.
   > **Исправлено 2026-09-20 (задача 6).** Это шире правды: смоук-раунд задачи 13b (`ab2d063`,
   > 2026-09-16, санкционирован владельцем и **не** приёмка) получил показание с устройства по
   > отгруженному коду **фаз 1–2** — см. первый абзац ADR. Верная формулировка: фазы 1–2 получили
   > показание в смоук-раунде, который не является приёмкой; **фаза 3a — весь действующий набор,
   > карточка `CONFIRM`, адаптер `launcher_memory` и каждый триггер `tr`/`en` — на телефоне не
   > запускалась вовсе, и фаза 3b тоже**. `DEVICE-ACCEPTED` **ОТСУТСТВУЕТ, а не неприменим**.
2. **Фаза 3b — одиннадцать навигирующих инструментов — не начата.** Спека §7.6 описывает её как
   однородную, повторяющуюся, субагентно-нарезаемую работу; итог по блоку при её завершении обязан
   докладываться **разбитым** на действующие и навигирующие (шестнадцать авторских, из которых пять
   действуют), никогда одной цифрой — это ровно та арифметика, которую §7.6 уже один раз поймала за
   двойным счётом `set_alarm`.
   > **Обновлено 2026-09-20.** Фаза 3b выполнена; раздел в конце ADR. И одна оговорка к самой цифре:
   > «шестнадцать авторских» — это инструменты, которые построил **блок A1″** (пять в 3a плюс
   > одиннадцать в 3b), формулировка спеки §7.6. **В федерации авторских инструментов двадцать**,
   > потому что четыре существовали до блока (`launch_app`, `play_store_search`, `set_timer`,
   > `open_system_settings`). Без слова «новых» шестнадцать читается как итог федерации и занижает
   > его на четыре.
3. **Финальное ревью всей ветки не проводилось.** Скилл SDD требует его при завершении плана; оно
   намеренно оставлено владельцу как решение о трате, потому что план фазы 0–2 всё ещё содержал Task 13,
   а ревью всей ветки — крупный расход, которому место рядом с ADR на закрытии блока.
4. **`DOC-HMA-2` этим блоком НЕ закрыт** (наследуется от A1′): уровни инструментов существуют, но
   останавливает ли цикл именно **смена** уровня — по-прежнему не построено. Долг A4′.
5. **Торч, громкость, медиа-транспорт и DND не отвергнуты — они не понадобились.** При пяти действующих
   инструментах пол выполнен с запасом и **новый раунд измерений на устройстве не потребовался**. Адрес:
   блок, которому следующим понадобится управление состоянием устройства.

---

### Фаза 3b — одиннадцать навигирующих инструментов (2026-09-20, `CODE-GREEN`)

**Раздел внутри этого ADR, а не новый ADR** — по прецеденту A1′, где приёмка владельца стала разделом
внутри собственного ADR блока. 3b выполнена отдельным заездом по плану
[docs/superpowers/plans/2026-09-19-a1-phase3b-navigating-tools.md](../docs/superpowers/plans/2026-09-19-a1-phase3b-navigating-tools.md)
(семь задач: перенос ловушек в код, три вертикальных среза, мутационный раунд, исправление
переобъявления, закрытие). Устройство не использовалось и не требовалось: спека §7.6 утверждает, что
«каждая Android-премисса, на которой стоит фаза 3, уже лежит в строках 13–33» — план проверил это
**построчно** по
[файлу измерений](../docs/superpowers/plans/2026-09-12-a1-device-measurements.md), а не по прозе
спеки, и нашёл при этом десять расхождений (ниже).

**Что построено.** Одиннадцать навигирующих инструментов в существующем `Tier0IntentToolSource`,
уровень `system_intent`, все `EXTERNAL` / `SAFE` / `TRANSIENT`: `show_alarms`
(`AlarmClock.ACTION_SHOW_ALARMS`), `open_camera` (`MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA`),
`open_wifi_settings`, `open_bluetooth_settings`, `open_battery_settings`
(`Intent.ACTION_POWER_USAGE_SUMMARY`), `open_data_usage_settings`, `open_display_settings`,
`open_sound_settings`, `open_location_settings`, `open_notification_settings` и `open_app_info`
(`Settings.ACTION_APPLICATION_DETAILS_SETTINGS` + `Uri` пакета) — единственный из одиннадцати, несущий
аргумент. Каждый: дескриптор, строка в `ToolPermissionCatalog.ROWS`, ветка `when` в
`Tier0IntentToolWorker` со **своим** `Intent(...)` на месте (см. P9), запись в `ToolVocabulary` в
`en`/`ru`/`tr`, литерал + ветка `toolLabelFor` + строковый ресурс в трёх локалях. Ни один инструмент не
остался зарегистрированным-но-недостижимым: каждая из трёх задач-срезов доводила свои инструменты до
поверхности внутри себя.

**`SAFE` обоснован положительно, один раз, и не словами «это же просто экран настроек»:** эти
инструменты **не совершают действия вообще**, поэтому отменять нечего, и «последний акт — за
пользователем» верно о них буквально. Это то же рассуждение, которое KDoc `Tier0IntentToolSource` уже
изолировал для `open_system_settings` и прямо отказался складывать с четырёхсвойственным обоснованием
`set_timer`. P3 и P5 его не ослабляют: то, что **приземлённый экран** предлагает пользователю, — не
действие, совершённое инструментом.

**Счёт — разбитый, всегда, и с оговоркой о том, какое из двух чисел какое.** Блок A1″ построил
**шестнадцать авторских инструментов, из которых действуют пять** (пять в 3a: `set_alarm`,
`uninstall_app`, три инструмента `launcher_memory`; одиннадцать навигирующих в 3b) — это формулировка
спеки §7.6 («Total new authored tools across 3a and 3b: sixteen, of which five act»). **В самой
федерации авторских инструментов теперь двадцать**, потому что четыре существовали до блока:
`launch_app` и `play_store_search` (уровень `in_app`, проекция `ActionCatalog`) и `set_timer` /
`open_system_settings` (`system_intent`, A1′). Разложение: `in_app` 2 + `system_intent` **15** +
`launcher_memory` 3, плюс динамическая семья `app_shortcut`, чьи имена — данные, а не литералы.
Проверено по дереву: `DoctrineGuardTest.REQUIRED_TOOL_IDS` содержит ровно эти двадцать
идентификаторов. **Отдельно названо потому, что «шестнадцать» без слова «новых» читается как итог
федерации и занижает его на четыре** — ровно тот класс ошибки, который §7.6 уже один раз поймала за
двойным счётом `set_alarm`.
...
**exit 0**, `557 actionable tasks: 557 executed`, прогон с **очищенного** `build/test-results`, числа
прочитаны из JUnit XML: **1439 тестов / 0 падений / 0 ошибок** (база 3a — 1438). Разложение:
`:domain` 440 · `:consumer:jvm` 56 · `:data:repository` **375** · `:feature:launcher` 194 · `:app` 59 ·
остальные восемь модулей 315 (`core/ui` 128, `data/ai-cloud` 32, `data/prayer` 38,
`feature/assistant` 38, `feature/settings` 33, `feature/prayer` 19, `feature/permission_education` 15,
`core/android` 12). **+1 — это весь прирост, и он объясним ровно одним новым `@Test`:**
`open_app_info takes exactly one app argument and it is required` в `Tier0IntentToolSourceTest`.
Одиннадцать инструментов дали ноль новых тестов не по недосмотру, а потому что существующие гуарды —
равенства над списками и свёртки над продакшн-федерацией: одиннадцать инструментов расширяют их
**входы**, а не добавляют методы. Декомпозиция выведена **до** прогона и сошлась.

#### Десять преднаходок — найдены чтением дерева, не прозы

**P1 — `Settings.ACTION_NOTIFICATION_SETTINGS` в платформе не существует, и наивное исправление красит
гуард.** Строка 25 измерений: `javap` по `platforms/android-37.0/android.jar` такого поля не находит.
Запись `Intent("android.settings.NOTIFICATION_SETTINGS")` роняет **половину полноты разбора** в
`ToolPermissionManifestGuardTest`: `anyIntentConstruction` считает каждый `Intent(`, а `actionsIn`
читает только то, что ловит группа `[A-Za-z_][A-Za-z0-9_.]*`, и строковый литерал не ловится — счётчики
расходятся. **Решение: регулярку НЕ расширять.** Объявлен `private const val
ACTION_NOTIFICATION_SETTINGS` в `Tier0IntentToolWorker` с комментарием, называющим строку 25, и
написано `Intent(ACTION_NOTIFICATION_SETTINGS)`: скан читает токен, карта ключуется на
`"ACTION_NOTIFICATION_SETTINGS"`, нового слепого пятна не открыто. Гуард повёл себя ровно как задуман —
«учить скан по одной настоящей форме за раз».

**P2 — разрешение `open_location_settings` измерено ДОСТАТОЧНЫМ и ни разу НЕОБХОДИМЫМ, и два
возможных варианта строки имеют противоположные режимы отказа.** Строка 24 прямо говорит, что
требование не измерено; строка 30 убирает *грант* из объяснений (`ACCESS_FINE_LOCATION: granted=false`
в момент открытия экрана), строка 33 подтверждает это изнутри процесса, строка 29 повторяет на свежей
сборке. Неисключённым остаётся *объявление*. Вариант `listOf("android.permission.ACCESS_FINE_LOCATION")`
означал бы, что `Tier0IntentToolSource.available()` — он фильтрует по `presence::isGranted` — **отнял бы
инструмент на телефоне самого владельца**, где строка 33 измерила это разрешение DENIED в процессе:
инструмент нигде не зарегистрирован, недостижим никогда, а списки-равенства в
`Tier0IntentToolSourceTest` остались бы зелёными (P6). **Решение: `emptyList()` с оговоркой,
записанной в комментарий строки**, потому что гуард способен проверить только те разрешения, которые
строка *заявляет*. **Это названный предел, а не закрытый вопрос**, и он вынесен нумерованным пунктом в
чеклист приёмки: если экран не откроется на сборке, где объявления от молитвенной фичи нет, строка
неверна.

**P3 — `open_wifi_settings` — единственный навигирующий инструмент, чей приземлённый экран менял
состояние устройства в одно касание, и делал он это на телефоне, через который владелец раздаёт
интернет.** Строка 18: экран Wi-Fi открылся **с уже поднятой модалкой** «Отключить мобильную точку
доступа? … Отмена / OK», потому что точка доступа была включена. Инструмент остаётся `SAFE` (он открыл
экран; модалка — ответчика, касание — пользователя), но это (a) комментарий на дескрипторе и (b)
**строка SAFETY** в чеклисте приёмки, а не рядовой нумерованный пункт.

**P4 — `open_battery_settings` резолвится в Samsung Device Care** (`com.samsung.android.lool/…
PowerUsageSummary`), а не в `com.android.settings` (строка 20). Само то, что
`ACTION_POWER_USAGE_SUMMARY` вообще резолвится, — **ROM-зависимая премисса, измеренная на одном
устройстве**; `launch()` уже возвращает `Failed` на `ActivityNotFoundException`. Названо в комментарии
дескриптора, не «исправлено».

**P5 — `open_app_info` открывает экран в одном касании от «Удалить»** (строка 15), и второй прогон
измерил, что пакет **без launcher-активности** не отфильтровывается. Рассуждение то же, что в P3:
`SAFE`, названо. Вместе с P3 это и есть содержательная половина обоснования `SAFE` выше — предложение
приземлённого экрана не есть действие инструмента.

**P6 — списки `Tier0IntentToolSourceTest` — это `assertEquals` над целыми списками, и именно это
заставляет R14-31 кусаться, а не защищает от него.** Равенства краснеют на **зарегистрированный**
дескриптор, но `available()` фейлится закрыто на отсутствующей строке каталога
(`catalog.permissionsFor(id) ?: return@filter false`), поэтому дескриптор **без** строки в
`ToolPermissionCatalog.ROWS` просто отнимается, `all()` возвращает прежний список, и все равенства
остаются **ЗЕЛЁНЫМИ**: инструмент нигде не зарегистрирован, а набор молчит. **Метод-правило, обязательное
для каждой задачи-среза: сначала расширять списки ожидаемых идентификаторов**, только потом писать
дескриптор и строку каталога. Тогда забытая строка — красный тест, а не невидимый инструмент. Это
механический ответ на «процедурный» статус R14-31, и он стоит ноль.

**P7 — взаимное затенение среди авторских записей уже ловится механически, по триггеру.**
`ToolVocabularyReachabilityTest.every trigger recognises its own sample command` утверждает
`vocabulary.match(command)?.id == entry.id`, а `ToolVocabulary.match` возвращает `null`, когда на один
текст претендуют две записи. Проверено мутацией (ниже).

**P8 — скан поверхности требует СЫРОГО строкового литерала для каждого идентификатора, не объявленного
в `domain/tool/ToolId.kt`.** Поэтому каждому из одиннадцати нужен `private const val` в
`AgentSessionPresentation.kt`, ветка `toolLabelFor` и строковый ресурс — форма, которую уже используют
`TIER0_SET_ALARM` / `TIER0_UNINSTALL_APP`, и по заявленной причине: у `:feature:launcher` нет ребра в
`:data:repository`.

**P9 — общий intent-хелпер вынул бы все одиннадцать из-под `ToolPermissionManifestGuardTest`
целиком, а одиннадцать почти одинаковых интентов — это ровно тот случай, когда хелпер пишут.** Гуард
допускает файл, только если тот и объявляет `: ToolWorker`, и содержит `Intent(` — хелпер не является
ни тем, ни другим (его KDoc называет это измеренным: проба `call_number` прошла **зелёной 4/4** при
отсутствующем в манифесте `CALL_PHONE`, и называет массу Tier-0 в A1″ конкретным риском). **Решение,
сказанное вслух: хелпера нет.** Все одиннадцать веток живут в существующем `Tier0IntentToolWorker`,
каждая строит свой `Intent(...)` на месте; нового файла воркера тоже нет, поэтому
`expectedIntentIssuingWorkers = listOf("Tier0IntentToolWorker.kt")` не тронут. **Если следующей задаче
захочется хелпер — это эскалация, а не рефакторинг.**

**P10 — Class B проверен на логике гейта, а не на файле.** `app/build.gradle.kts` фильтрует
`it.name == "strings_locked.xml" && it.parentFile.name == "values"`;
`grep -c launcher_agent_step …/values/strings_locked.xml` → **0**. Шаговые строки живут в
`values/strings.xml`, вне области подписи. Итог: **ничего не переподписывалось и никакой sha256 агентом
не считался.**

#### Второй подряд per-descriptor пин `required` — и обобщения по-прежнему нет

`open_app_info` получил собственный тест, утверждающий, что его аргумент `app` объявлен
`required = true`. Это **второй такой пин подряд**: первым был `uninstall_app` (3a, Task 7, R14-35).
Оба существуют из-за одного и того же зазора — `ToolMatchPlanner` входит в ветку разрешения, ключуясь
на **имени** аргумента (`app`), а не на его `required` (остаточное ограничение (1) блока: дескриптор,
объявивший `app` необязательным, даёт `raw = ""` → `resolve("") = null` → `NoPlan` для любой цели,
навсегда, при зелёном наборе). **Два экземпляра, ноль обобщений** — и именно накопление экземпляров, а
не отдельный пин, является свидетельством, которое нужно A4′: это тот же долг `ArgType` (спека §7.5),
пересказанный на уровне планировщика вместо уровня схемы.

#### Турецкий: семь → ДВАДЦАТЬ ТРИ, и число было занижено в собственном плане

По соглашению, которым считает остаточное ограничение (4) блока (`için`/`kullan` — один пункт), после
3b неосуждённых носителем турецких форм **двадцать три**; как сырых различных строк триггеров — **24**.
Таблица словаря держит **27** форм `tr`, из которых три (`zamanlayıcı ayarla`, `sistem ayarları`,
`android ayarları`) предшествуют этому учёту. Прирост — шестнадцать строк на одиннадцать инструментов,
потому что большинство новых записей объявляют по **две** формы `tr`.

**§0.3 плана сначала сказала «восемнадцать», и это была ошибка занижения**, выведенная как 7 + по одной
форме на инструмент. Исправлено в плане после сборки и записано здесь, потому что **занижённый дефицит
— тот же класс ошибки, что и завышенный успех**, а это ровно предмет задачи 6 того же плана. Наименее
уверенные формы названы заранее, как требует дисциплина R14-42: `pil ayarları`, `veri kullanımı`,
`bildirim ayarları` (плюс `konum ayarları` и `uygulama bilgisi` как новые кандидаты в чеклисте).
«Предполагается, что нормально» — не приемлемый ответ на эту строку приёмки.

#### Поверхность гуард-листов осталась на четырнадцати — и это НЕ доказательство её перечислимости

Преднаблюдение плана перечислило **четырнадцать** списков, которые обязаны двигаться вместе при
добавлении инструмента (двенадцать из брифа плюс найденные сканом `ToolVocabularyLocaleGuardTest` и
`ToolVocabularyReachabilityTest.REQUIRED_TOOLS`). 3b добавила одиннадцать инструментов и **ни одного
нового списка**: пункт 12 (`ToolWorkerCallSiteGuardTest`) не сработал, потому что P9 запретил новый
файл воркера. **Это не свидетельство того, что множество перечислимо заранее** — только того, что эта
фаза не добавила **формы** инструмента, которой поверхность ещё не научена (все одиннадцать — тот же
`system_intent`, тот же воркер, тот же уровень). Правило R14-41 в силе: поверхность «добавить
инструмент» нельзя перечислить из документа, её можно только обнаружить прогоном гейта.

#### Мутационный раунд — три находки, и они ценнее одиннадцати инструментов

1. **Пин `required = true` у `open_app_info` не охраняет тот режим отказа, который называло его
   сообщение.** С подсаженным `required = false` планировщик **всё равно планировал верно**: словарь
   объявляет `argName = "app"` и отказывает голому триггеру (`match("app info")` → `null`), поэтому
   `raw` никогда не пуст, и флаг меняет лишь то, **какая строка** вернёт `NoPlan`. Пин фальсифицируем —
   ложным было его обоснование. **Это ВОСЬМОЙ случай собственного паттерна блока** — свидетельство,
   описывающее не то, что от него думают. Сообщение ассерта исправлено в дереве на истинное, более
   узкое свойство: пин стоит против изменения **проводки**, а режим отказа R14-37 возникает у любого
   дескриптора, чей словарь `app` **не** поставляет, — в одной правке словаря отсюда.
2. **Авторская ветка `DoctrineGuardTest` несёт ту же слепоту `String.contains`, которая раньше была
   измерена только у динамической ветки.** Удаление ветки `toolLabelFor` при оставленном на месте
   `private const val` оставило `:app` **ЗЕЛЁНЫМ** на 59/0 — инструмент рисовал бы generic-строку шага
   вечно. Удаление ветки **и** константы даёт красный. KDoc этого файла раньше утверждал: «авторская
   ветка выше этой находкой не ослаблена» — **предложение измерено ложным и исправлено в дереве**.
   Форма реалистична, а не экзотична: осиротевшая `const` — предупреждение Kotlin, а
   `allWarningsAsErrors` эта сборка не включает. **Не починено**: закрытие означает читать ветки, а не
   текст файла. → A4′ или тот блок, который следующим тронет этот гуард.
3. **Тест, чувствительный ко времени, способен покрасить граничный прогон по причине, не связанной с
   диффом.** `:data:ai-cloud`'s `first token arriving just under the deadline still completes normally`
   упал один раз под спина-к-спине нагрузкой Gradle и прошёл на немедленном повторе с очищенного
   дерева; тот же падавший прогон оборвался на 407/557 задач, то есть был ещё и неполным. **Тот же
   класс, что R14-44** — сигнал гейта, описывающий не то, что от него думают. Записано, не починено.

Остальные четыре мутации раунда — взаимное затенение двух новых триггеров (P7), дескриптор без строки
каталога (P6/R14-31, где метод-правило «списки первыми» и должно было устоять), сырой строковый
литерал вместо константы действия (P1) и ложный `emptyList()` при заявленном в строке, но отсутствующем
в манифесте разрешении (`ToolRegistryPermissionGuardTest`) — **по отчёту задачи 5** дали ожидаемый
красный и были откачены. Их протокол не лежит в дереве (леджер `.superpowers/sdd/` для 3b не заведён,
коммитов у фазы до этого момента нет), поэтому здесь они записаны как отчёт задачи, а не как измерение,
подтверждённое этим разделом; три находки выше — то, что задача 5 отчитала как расхождения, и они
подтверждены в дереве исправленными текстами ассерта и KDoc.

#### Чего фаза 3b не сделала — сказано, а не подразумевается

1. **На телефоне не запускалось ничего из построенного в 3b.** Ни один из одиннадцати экранов, ни одна
   турецкая или английская форма. Статус фазы — `CODE-GREEN`.
2. **Критерий 1 спеки §16 («≥15 инструментов из ≥3 уровней, достижимых на телефоне владельца, приёмка
   владельцем») по-прежнему НЕ выполнен** — но теперь ровно по одной причине: приёмки владельца не
   было. Инструментов и уровней хватает: двадцать авторских на трёх уровнях плюс динамическая семья
   `app_shortcut`; достижимы из текста восемнадцать авторских (все, кроме `launch_app` и
   `play_store_search`, которые достижимы планом A0, а не собственным триггером). Приёмка — Фаза 4,
   владельца, по
   [чеклисту](../docs/superpowers/plans/2026-09-19-a1-second-device-acceptance.md), в который 3b
   дописала одиннадцать строк, строку SAFETY по P3 и обновлённую турецкую строку.
3. **Ни одно остаточное ограничение A4′ не чинилось** — по инструкции плана «накапливать
   свидетельство, не чинить»: R14-37 (обобщение), R14-38, R14-39, `DURABLE_EFFECT`, настенные часы.
   3b добавила к этой копилке второй пин `required` и вторую слепоту `String.contains`.

---

### Приёмка на устройстве — прогон владельца 2026-09-20 (SM-A325F / Android 13; `ru-RU`, плюс впервые `tr` и `en`): блок становится `CLOSED`

**Раздел внутри этого ADR, а не новый ADR** — тот же приём, что и у фазы 3b выше, и тот же прецедент:
приёмка A1′ записана разделом внутри собственного ADR блока. Владелец выполнял пункты сам; агент
готовил сборку, вёл прогон, читал инструменты и записывал. Прогон `adb`/`uiautomator` под управлением
агента приёмкой не является (словарь Этапа 0.5), и ни один пункт ниже на такой прогон не опирается.

**Какой билд подписан: `db1e75d`, НЕ исправленный, и это решение владельца, а не умолчание.** Здесь
расхождение с прецедентом A1′, где приёмка произвела три починки и владелец подписывал уже
исправленную сборку (`1ef158d`). Этот раунд нашёл дефект продукта (находка 1 ниже) — владелец
постановил **не чинить его в этом блоке** и вынести в A4′. Поэтому принятое поведение — это поведение
`db1e75d` **вместе с названным дефектом**, и `CLOSED` здесь тем более не означает нулевой долг
(прецедент DS-6B).

**Гейт, которым мерили.** Снят до установки, с **очищенного** `build/test-results`, декомпозиция
выведена **до** прогона: **1439 тестов / 0 падений / 0 ошибок**, `557 actionable tasks: 557 executed`.
Помодульно сошлось точно: `:domain` 440 · `:consumer:jvm` 56 · `:data:repository` 375 ·
`:feature:launcher` 194 · `:app` 59 · остальные восемь 315. Чувствительный ко времени тест
`:data:ai-cloud` на этом прогоне прошёл. **За весь раунд агент не изменил ни одного файла
репозитория**, поэтому этот гейт описывает ровно тот билд, который принимался.

**Состояние устройства.** SM-A325F, Android 13, `ru-RU` — та же машина и та же локаль, что у Task 15
(A0), у пере-проверки 2026-08-23 и у приёмки A1′. Debug-сборка ставилась **поверх** существующей:
`uid` остался **10752**, `firstInstallTime` не изменился (2026-09-14), `databases/` цел
(`sidr_history.db` 81920 + `-wal` 420272 байта). Схему блок не менял, миграция не запускалась и не
должна была. Оба install-time разрешения (`SET_ALARM`, `REQUEST_DELETE_PACKAGES`) выданы молча, ни
одного экрана разрешений установка не подняла. Роль `HOME` на старте держал лаунчер Samsung —
это и есть состояние «роли нет», и §A7 проверил его первым.

**Наблюдение, сделанное ДО установки и опровергающее половину записанного обоснования P2:**
`ACCESS_FINE_LOCATION` на этом телефоне сегодня **`granted=true`** (`USER_SET`), тогда как строки
30/33 файла измерений писались при `granted=false`. Довод «объявить это разрешение значило бы
заставить `Tier0IntentToolSource.available()` **утаить** инструмент на этом самом телефоне» на текущем
состоянии устройства **несостоятелен**. Отгруженный `emptyList()` от этого не становится неверным —
но обоснование под ним устарело, и ограничение (11) остаётся названным, **не** суженным этим прогоном.

#### Часть A — то, на что владелец обязан посмотреть глазами. Пройдена целиком

1. **§A1 `uninstall_app`.** План остановился, карточка согласия нарисована, ничего не выполнилось.
   Карточка назвала **и ярлык, и пакет**, причём пакетом оказался `org.telegram.plus` — форк Plus
   Messenger, а не `org.telegram.messenger`, который предполагал чеклист. **Это оказалось лучшей
   проверкой условия 2, чем задуманная:** назови карточка только ярлык, владелец не узнал бы, что
   удалиться собирается форк. Провенанс `SYSTEM INTENT · EXTERNAL` на месте. «Закрыть» не выполнила
   ничего и вернула домашнее тело с **очищенным** буфером команды — починка (b) приёмки A1′ не
   отъехала. «Подтвердить» подняла системный диалог удаления; владелец его отменил, **ничего не
   удалено** (проверено: `pm list packages` по-прежнему видит пакет).
2. **§A2 самоудаление.** Прогнан владельцем лично после исправления чеклиста (см. дефект чеклиста 1):
   имя разрешилось в наш собственный пакет, карточка `CONFIRM` его назвала, отказ пришёл **после**
   подтверждения. **R14-38 впервые наблюдался на устройстве.**
3. **§A3 неразрешённое имя.** Карточка не рисуется — планировщик отказывается, а не угадывает. Вторая,
   доктринальная половина пункта разобрана ниже отдельно: она стоила раунду двух исправлений записи.
4. **§A4 `set_alarm`.** Будильник создан и **уже включён**, время совпало, план одношаговый
   выполненный, провенанс на месте.
5. **§A5 три инструмента `launcher_memory`.** Все три без карточки согласия, строка шага называет
   аргументы, псевдоним реально появился в Настройки → Память и реально **изменил разрешение**
   (голая «телега» открыла приложение), а «забудь название телега» его снял — тихого отказа
   «сохранено по нормализованной фразе, удаляется по сырой», против которого набор написан, **не
   произошло**. Потребовало двух исправлений чеклиста (дефекты 2 и 3).
6. **§A6 двадцать три турецкие формы — ГЛАВНЫЙ результат раунда.** Владелец взялся судить их сам и
   **принял все 23**: «все работает, значения перевода верные», «все проверено и принято, все чётко».
   Это закрывает строку, открытую с 2026-09-10, **включая унаследованный `sayaç ayarla`**, который
   A1′ честно оставил несудимым. Правки `ToolVocabulary.kt` не требуются — вердикт «выбросить» был бы
   изменением кода, а не документа. **Впервые за проект турецкие формы судил человек, и впервые `tr`
   вообще выполнялся на телефоне.** Попутно установлен механический факт, удешевивший пункт вдвое:
   `ToolVocabulary.matchIn` делает `prefixByLocale.values.flatten()`, то есть **сопоставление
   триггеров локали не знает** — формы всех локалей активны одновременно, и турецкие триггеры
   проверяемы из `ru-RU`; переключение локали нужно только чтобы читать **строки**.
7. **§A7 адаптер `app_shortcut`.** Обе половины. Без роли: ярлык не запустился, лаунчер не упал,
   источник не рекламировал ничего. С ролью: «youtube shorts» и «plus Азат» открыли ярлыки. Три
   проверки отбора прошли — голое имя ярлыка **не** совпадает без названного приложения; авторский
   триггер «системные настройки» **не затенён**; «системные настройки для моей машины» не запускает
   **ничего**. «plus Избранное» не сработало **не из-за дефекта**: такого ярлыка приложение не
   публикует, и отбор корректно отказал — что доказано тем, что на том же билде и в том же процессе
   «plus Азат» сработал.
8. **§A8 строки.** Прочитаны в `tr` и `en` **выборкой** (одна команда каждого типа), а не полным
   повторным проходом по двадцати командам дважды — это названо ограничением раунда, а не выдано за
   полный проход. Непереведённого, обрезанного и похожего на отладочную строку не найдено; токены
   провенанса остались английскими; переподписывать ничего не просили.
9. **§A9 одиннадцать навигирующих инструментов.** Все одиннадцать открыли свои экраны, ни один не был
   перехвачен другим, ни один не сел на `open_system_settings` вместо своего. `open_notification_settings`
   открылся — значит **наша собственная** строка действия (константы `Settings.ACTION_NOTIFICATION_SETTINGS`
   в платформе нет, проверено `javap`) на этом ROM разрешается. `open_battery_settings` открылся —
   ROM-зависимая премисса P4 (Samsung Device Care) **держится** на этом устройстве.
   `open_location_settings` открылся — но см. выше: этот прогон **не различает** «не нужно ничего» и
   «нужно объявление, которое и так есть», потому что разрешение сегодня выдано. Правило безопасности №4
   соблюдено: Wi-Fi открылся с уже поднятой модалкой «Отключить мобильную точку доступа?», владелец
   закрыл её «Отменой», **OK не нажимался**, раздача на ноутбук не прерывалась ни разу за раунд.
   Ограничение читаемости `open_app_info` (строка шага рисует **разрешённый пакет**, а не набранное
   слово) владелец посмотрел прицельно и **принял явно**: «все ок означает все хорошо работает и я
   принимаю».

#### Часть B — регрессы. Пройдена целиком

Двухшаговый план A0 доехал как прежде; оба инструмента A1′ (`set_timer`, `open_system_settings`)
работают; FastPath не тронут — установленное приложение открывается без захода на агентную поверхность;
**негативная проверка `alarm kur` прошла: она ушла в магазин, а не завела будильник** — намеренное
исключение этой формы из турецкого набора держится. Выход с агентной поверхности очищает буфер.
Каскадное удаление проверено **правильным** чтением — `tools/device/pull-agent-db.sh`, с WAL: в покое
`agent_session=0 · agent_plan_step=0 · agent_trace_event=0`. Это ровно тот пункт, на котором приёмка
A1′ споткнулась, и споткнулась тогда **на чтении**, а не на движке.

#### Семь находок прогона

**1. Отменённое удаление рисуется как «выполнено». Дефект продукта, доктринальный.** Владелец отменил
системный диалог — и увидел «Удалить — **выполнено**», при том что приложение на месте. Корень прочитан
в дереве: `Tier0IntentToolWorker.uninstallApp` вызывает `launch(Intent(ACTION_DELETE, …))`, а `launch()`
после `launcher.launch(intent)` безусловно возвращает `ToolResult.Effected()`. То есть `Effected`
возвращается за то, что **диалог был показан**, а не за то, что приложение удалено; система спрашивает
**всегда**, и пользователь **всегда** может отказаться. Нарушены `DOC-ILM-3` («трасса 1:1 с
реальностью») и `DOC-ILM-4` («частичный результат показан как частичный, а не как успех») — причём
соседняя функция того же файла (`openAppInfo`) цитирует первое из них дословно: «answering
`ToolResult.Effected` for an effect that cannot have happened is what `DOC-ILM-3` forbids».
**Это первый инструмент такого рода:** `set_timer`/`set_alarm` будильник реально заводят (строки
13/29), одиннадцать навигирующих реально открывают экран, `play_store_search` реально открывает
магазин — `uninstall_app` **первый, у кого `Effected` может быть ложью**, и он же единственный
необратимый. **Почему не починено:** честного значения «передано системе, исход неизвестен» в словаре
**нет** — `ToolResult` это `Effected | Observed(ObservedFact) | Failed`, а `ObservedFact` заморожен
(§16 критерий 7 и раздел «Do not»). Это находка A0.5 «целый класс реальности невыразим», пришедшая к
оплате. Владельцу предъявлены три варианта с ценой: назвать остаточным; починить формулировку шага
(чинит **экран**, оставляя **трассу** лгущей); починить словарь результата (замороженный тип, отдельный
блок). **Постановление владельца: вариант 1** — «решим эту проблему в A4′». Отгружается как названное
остаточное ограничение.

**2. R14-38 наблюдался и оставлен на месте.** «удали приложение sidr launcher» разрешается в наш пакет,
рисует карточку `CONFIRM` с его именем, и только потом воркер возвращает `Failed`. **Постановление
владельца: оставить как есть**, адрес — A4′.

**3. R14-39 подтверждён на устройстве — но запись пришлось исправлять дважды, и это самая
поучительная часть раунда.** Сначала «удали приложение панголин» дала «Неизвестная команда», и агент
записал это как доказательство ухода текста в облако. Затем **владелец сообщил, что ключ API был введён
неверно** — и запись была понижена до «маршрут до модельного пути пройден, egress **не измерен**»,
потому что провод не снимался, а `RouteCommandUseCase:172` сворачивает **любой** сбой планировщика в
тот же `NoPlan`. После того как владелец поставил рабочую модель, пункт закрыт **наблюдением**:
«включи plus» — глагол, которого нет ни в `LAUNCH_VERBS` FastPath'а, ни в одном авторском триггере, —
дошёл до шага 7, модель предложила запуск, «Выполнить» его исполнило. **Модельный путь на этом билде
жив end-to-end**, значит «удали приложение пантолин» шёл тем же маршрутом: при настроенном провайдере и
живой сети других дорог до «Неизвестной команды» нет (`RouteCommandUseCase:179`). Модель не смогла
отобразить команду в каталог **правильно** — действия «удалить» среди семи `ActionIds` нет вовсе.
**Итог:** цель, которую зарегистрированный инструмент уже сопоставил детерминированно, ушла сырым
текстом в облако из-за неразрешённого имени, **и пользователь не получил об этом никакого признака**.
Строгость формулировки: провод для той конкретной команды не снимался; наблюдается, что путь жив и что
команда доказуемо им прошла. Починка — A4′.

**4. Выдача роли `HOME` живому процессу не видна. Новая находка, сужает принятый критерий §16 №2.**
Владелец передал роль Sidr при работающем приложении — и «youtube shorts», команда, **доказанно
работавшая на этом телефоне** в смоук-раунде `ab2d063`, ответила «неизвестная команда». Агент сделал
`force-stop` и перезапуск (без переустановки, данные не тронуты) — та же команда открыла ярлык. Корень:
`ShortcutRefreshTrigger.start()` выполняется **один раз на процесс**, регистрирует наблюдателя
**изменений ярлыков** и делает первый `catalog.refresh()`; `ShortcutCatalog` — снимок. **Смена роли
изменением ярлыков не является**, колбэка не приходит, снимка не обновляется. Критерий §16 №2 («набор
инструментов, меняющийся при живом процессе, виден через оба лица федерации») **верен для федерации** —
она выводит оба лица на каждый вызов, — но **не для источника под ней**, когда меняется **роль**.
Критерий этим **сужен, а не опровергнут**: он написан про федерацию. **Почему смоук-раунд это
пропустил:** S7/S8 мерили направление «роль **отобрали**», причём зондом в **свежем** процессе, так что
живой переход роли не проверялся **ни в одну** сторону. Сегодняшний — первый, и он в направлении
«выдали». Не чинилось (приёмка не место для проектирования); кандидат — обновлять снимок и на смену
роли. Адрес: A4′ или блок, который следующим тронет адаптер.

**5. Неверная конфигурация BYOK молчит на командном пути. Находка, произведённая самой ошибкой с
ключом.** Неверный ключ, неверный id модели, недоступный эндпоинт и «модель ответила прозой»
сворачиваются в **один и тот же** `PlanResult.NoPlan` и дают неотличимое «Неизвестная команда». У
командного маршрутизатора **нет канала обратной связи** «провайдер нас отверг». Свидетельство прямое:
владелец вёл приёмку, считая провайдера настроенным, тогда как тот был непригоден, и продукт не сказал
**ничего**, что отличало бы это от «не понял». Контраст: путь ассистента/BYOK-чата ошибки транспорта
**показывает**, то есть две поверхности расходятся в том, стоит ли сообщать пользователю о сбое
провайдера. Тот же класс, что остаточное (3) A1′ и R14-39, **с третьей причиной**: провайдер настроен,
но непригоден. Адрес: A4′.

**6. Командная строка не отвечает на вопрос, а уходит в браузер. Разрыв против ADR 4/4, не дефект
A1″.** Enter по свободному тексту открывает веб-поиск. Это **не** падение FastPath — его финальное
падение это `UnknownIntent` с уверенностью 0.10, а не поиск. В браузер уводит **модель**: она
отображает вопрос в `ActionIds.WEB_SEARCH`, один из семи замороженных. Командная поверхность способна
произвести только **действие**, а лучшее доступное действие для вопроса — поиск. Это прямо расходится с
ADR 4/4 («один разговорный контур, две поверхности; план из 0 шагов **и есть** произнесённый ответ»):
действия «ответить» в каталоге нет, поэтому произнесённый ответ из командной строки недостижим и живёт
только за «СПРОСИТЬ». В каталоге есть `OPEN_ASSISTANT`, то есть модель **могла** увести вопрос к
ассистенту, но выбор ничем не направлен. Адрес: недостроенная половина ADR 4/4 (A4′/A6).

**7. Выход из приложения ведёт на «Недавние» Samsung, а не в Sidr. Свойство платформы, не дефект.**
Владелец заметил, что выход из открытого приложения даёт стандартный лаунчер, а через время — Sidr.
Агент воспроизвёл инструментально: после `BACK` из YouTube фокус — `com.sec.android.app.launcher/
com.android.quickstep.RecentsActivity` и остаётся там. Это **экран «Недавние»**, реализованный
**внутри пакета лаунчера Samsung**; на One UI сторонний лаунчер overview не поставляет, поэтому Recents
остаётся за Samsung даже при роли `HOME` у нас. Сам `HOME` разрешается в Sidr и держится (замерено на
t+0.3/2.3/5.3 с). Починить в лаунчере нельзя; записано как реальное ограничение «Sidr как повседневный
лаунчер на Samsung».

#### Четыре дефекта ЧЕКЛИСТА — и это отдельный результат раунда

Из пяти расхождений, вскрытых прогоном, **четыре оказались дефектами документа, а не кода**, и каждый
раз инструмент проверялся **до** объявления дефекта (прецедент A1′, где самой дорогой ошибкой раунда
был собственный способ чтения базы):

1. **§A2 называл мишень, которой на телефоне нет.** «удали sidr» не разрешается ни во что: ярлык
   приложения — **«Sidr Launcher»** (`strings_locked.xml`, `translatable="false"`), а
   `AppTargetResolver` матчит по **точному** нормализованному ярлыку и никогда не угадывает. Продукт
   отработал правильно, выдав честное «ИИ-провайдер ещё не настроен» (тогда его действительно не было).
   Верная команда — «удали приложение sidr launcher».
2. **§A5 строка 1 пропускала ИНФИКС, в `ru` и в `en` сразу.** `set_app_alias` — единственная запись с
   **двумя слотами**, форма `<префикс> A <инфикс> B`, инфикс ищется **окружённым пробелами**. Чеклист
   писал «называй telegram телега» и "call telegram tg", тогда как турецкая строка того же пункта
   («telegram için tg kullan») инфикс **содержит** — документ противоречил сам себе. Верная форма —
   «называй telegram **как** телега».
3. **§A5 требовал увидеть `LAUNCHER MEMORY · LOCAL`, который код намеренно НЕ рисует.**
   `provenanceLabelFor` начинается с `if (effect != ToolEffect.EXTERNAL) return null`; все
   `launcher_memory`-инструменты `LOCAL`, и KDoc прямо обосновывает: заявить пересечение границы,
   которого не было, значило бы «`DOC-ILM-2` лжёт в обратную сторону». Строка
   `launcher_tool_level_launcher_memory` отгружена **заблокированной и недостижимой** — существует
   ради тотальности отображения. Чеклист назвал «сутью уровня» ровно то, чего код рисовать отказывается.
4. **§A5 обещал, что «открой телега» откроет приложение.** Псевдоним по контракту «заполняет только
   пробелы»: `ResolveCommandWithAliasUseCase` срабатывает **только** на `CommandOutcome.Unknown` и ищет
   `store.find(normalize(rawInput))` по **всему** вводу. «открой телега» FastPath **решает**
   (глагол понят, приложения нет) → `NoAppFound` → декоратор пропускает → срабатывает ветка A0 →
   магазин. Голая «телега» открывает. Механизм цел, неверно обещание.

**Метод, который из этого следует, и он дороже самих правок.** Чеклист, написанный при закрытии блока,
ошибался **чаще, чем код, который он проверяет**, и три из четырёх дефектов — это утверждения о том,
что пользователь **увидит**, сделанные без единого прогона на телефоне. Это тот же паттерн, который
блок уже зафиксировал внутри себя восемью случаями — «свидетельство, описывающее не то, что от него
думают», — проявившийся теперь на уровне **приёмочного документа**. Правило на будущее: строка чеклиста,
предсказывающая конкретный видимый рендеринг, до первого прогона является **гипотезой**, и писать её
надо тоном гипотезы.

#### Чего раунд НЕ покрыл — сказано, а не подразумевается

- **§A8 прочитан выборкой**, а не полным повторным проходом по всем командам в `tr` и `en`.
- **Провод не снимался** ни разу: egress подтверждён рассуждением над живым путём, а не перехватом.
- **Часть C не исполнялась как отдельный раздел** — то, что из неё есть, собрано по ходу (гейт,
  проверка Class B, счётчики ярлыков, чтения базы) и **не закрывает ничего**; это прямо сказано здесь,
  как того требует пункт 5 «Закрытия» чеклиста.
- **P2 не сужен**: разрешение геолокации сегодня выдано, поэтому прогон не различает «не нужно ничего»
  и «нужно объявление, которое и так есть».
- **Пять из двадцати авторских инструментов** (`launch_app`, `play_store_search` и прочие,
  достижимые не своим триггером) проверялись планом A0, а не собственной командой — как и прежде.

#### Критерий 1 спеки §16 — ЗАКРЫТ

«≥15 реальных инструментов из ≥3 уровней зарегистрированы и достижимы на телефоне владельца, счёт
доложен разбитым, приёмка выполнена владельцем». Зарегистрировано **двадцать авторских инструментов на
трёх уровнях** плюс динамическая семья `app_shortcut` на четвёртом; достижимы из набранного текста
**восемнадцать** авторских. **Счёт, как и требуется, разбитый: блок A1″ построил ШЕСТНАДЦАТЬ авторских
инструментов, из которых ПЯТЬ ДЕЙСТВУЮТ; в федерации их ДВАДЦАТЬ**, потому что четыре предшествуют
блоку (`launch_app`, `play_store_search`, `set_timer`, `open_system_settings`). Приёмку выполнил
владелец. **Это был последний незакрытый из девяти критериев §16.**

**Статус блока: `CLOSED`** — гейт зелёный, приёмка владельцем выполнена, остаточные ограничения
**названы**. `CLOSED` не означает нулевой долг: к двенадцати ограничениям, записанным при закрытии
`CODE-GREEN`, этот раунд добавил находки 1, 4, 5 и 6 выше, а находка 7 записана как свойство платформы.
Ни одно из них не чинилось — по постановлению владельца и по правилу «накапливать свидетельство для
A4′, не чинить».

---

## 2026-09-20 — Две строки доктрины: грант не вычитает гейт, и три класса содержимого

**Status: принято.** Только документы, ноль production-кода. Это **не** блок A4′ и не его спека: это
первая из трёх работ подготовки Этапа 6 по плану брейншторма, единственная, которая не блокируется ни
владельцем, ни измерением.

**Change-control.** Master Plan §5 требует ADR на «добавить правило в доктринальную матрицу»; настоящий
ADR и есть это требование. **Закрытый словарь типов проверки §1 матрицы НЕ трогается** — это отдельный
пункт того же §5, и оба новых правила пользуются уже существующими типами. Ни одно существующее правило
не переформулировано и не поменяло тип.

**Почему сейчас, а не в A6, которому оба правила адресованы.** `docs/improvements-review.md` §6 назвал
правило о гранте первым из трёх «самых дешёвых ценных продвижений», и довод у него не риторический, а
механический: **список гейтов на пути замкнут и перечислен в Hard rules `CLAUDE.md`**, поэтому
утверждение «грант из него не вычитает» проверяемо в принципе, а не только декларируемо. Решение
владельца 2026-09-20: внести оба правила сейчас. Цена внесения — один ADR и две строки; цена отсрочки —
что change-control §5 продолжает расти списком, к которому дописывают, вместо принципа, из которого
выводят.

### `DOC-HMA-5` — грант расширяет досягаемость и никогда не снимает гейт

> Грант расширяет то, **куда агент дотягивается**, и никогда не снимает гейт, **через который он
> проходит**. Список гейтов на пути замкнут — реестр, валидация аргументов, предусловия, гейт риска и
> согласие, границы цикла, egress allow-list, трассировка — и грант из него не вычитает.

Источник — манифест `SidrOS` §13 и `docs/improvements.md` Часть 3 F. Правило разделяет два вида
«запрещено»: **первый** запрещён до разрешения пользователя (accessibility, чтение уведомлений,
удаление приложений) — это законно выдаётся грантом со скоупом, лимитами и отзывом; **второй** — то, на
чём стоит целостность системы (исполнение мимо реестра, пропуск валидации, пропуск allow-list, «не
спрашивай больше» на необратимом, цикл без бюджета) — и он не выдаётся никогда, иначе честным описанием
продукта становится «агент с гейтами, если их не выключили».

**Почему `HMA`, а не `ADL`.** Ближайший сосед по смыслу — `DOC-ADL-1` («одинаковый risk получает
одинаковый гейт независимо от источника предложения»), и соблазн поселить правило там реален. Но оси
разные: `ADL-1` говорит, что гейт **не меняется от того, кто предложил**; `HMA-5` — что гейт **не
исчезает оттого, что пользователь заранее разрешил**. Второе — ограничение на человеческую власть над
собственной системой, а §2 матрицы ставит `Human-authority` в не-негоциируемый ранг. Ближайший сосед по
форме — `DOC-HMA-4` («автономный фоновый цикл без видимости запрещён»): оба говорят «это запрещено
потому, что на этом стоит целостность», а не «это запрещено, пока не разрешат».

**Что правило сегодня НЕ закрывает, и это сказано вперёд, а не обнаружится потом.** Типа `Grant` в
дереве не существует (слой A6, ноль файлов), поэтому у половины «грант не вычитает» **нет субъекта**:
нечего проверять, пока нечего выдавать. Закрытой приезжает только половина «гейт нельзя обойти», и
держат её не эта строка, а `ToolExecutorCallSiteGuardTest` и `ToolWorkerCallSiteGuardTest` —
мутационно проверенные, по одной точке вызова каждый.

**В ячейку матрицы эти два имени НЕ вписаны, и это осознанно.** §5 матрицы фиксирует механику:
`DoctrineMatrixGuardTest` пропускает проверку существования у **всей** ячейки, содержащей `<нет>` —
значит имя настоящего теста, поставленное рядом с этим токеном ради грепа, молча выключило бы проверку
для самого себя. Ячейка поэтому несёт `<нет> ← долг A6` и прозу без имён, а связь с двумя гвардами
живёт здесь.

**Тип `arch-guard` — предсказание формы будущего теста, и предсказание уже ошибалось дважды.** Журнал §6
матрицы хранит обе ошибки: `DOC-ILM-3` и `DOC-ILM-2` записывались как `arch-guard`, потому что ожидался
сканирующий гвард, а закрыло их в обоих случаях поведение (`unit`). Тип выбран как наиболее вероятный
для правила, чьё содержание структурно («замкнутый список, из которого нельзя вычесть»), и если правило
закроет тест другой формы — тип исправляется тем же change-control, а не тихо.

### `DOC-ILM-5` — три класса содержимого не смешиваются

> **Каноническое** — точное, адресуемое хэшем, никогда не от модели. **Курируемое** — с показанным
> источником. **Сгенерированное** — только там, где «примерно верно» допустимо, и помечено как
> сгенерированное.

Источник — манифест §12 и `docs/improvements.md` Часть 3 C.

**Почему `ILM`, а не `AML`.** Правило о **правдивости показанного**, а не о приватности: оно отвечает на
вопрос «что вообще позволено предъявить пользователю как истину». `AML` целиком про egress и хранение;
`ILM` — про объяснимость и провенанс (`ILM-1` провенанс на поверхности, `ILM-2` провенанс у `EXTERNAL`
инструмента, `ILM-3` трасса 1:1, `ILM-4` частичное показано частичным). Пятое правило семьи продолжает
ровно эту линию: у содержимого тоже есть провенанс, и он тоже обязан доходить до человека.

**Обоснование исправлено при переносе, а не перенесено дословно — и это главное, что здесь произошло.**
`improvements.md` обосновывал таксономию двумя прецедентами репозитория: SHA-256-проверкой `ModelStore`
и bundled-индексом GeoNames. Проверка 2026-09-14 (`improvements-review.md` §2.2) нашла **оба полыми**:
`ModelStore` удалён Этапом 0.3 вместе со всем ONNX-стеком, и его воскрешение прямо стоит в Do-not-списке
`CLAUDE.md`, так что цитировать его как живой прецедент отдельно опасно; а у индекса GeoNames проверки
целостности **нет вообще** — он bundled, и целостность берётся от подписи APK, то есть он в принципе не
прецедент для проверки **загруженного** содержимого, ради которой его назвали.

Правило поэтому вносится как требование к будущему с записанным «механизма у нас нет», а не как
кодификация существующей практики. Первый элемент из сети обязан построить эту проверку с нуля.
Заявить прецедент, которого нет, значило бы совершить ровно ту ошибку, ради ловли которой матрица и
существует.

### Что этот ADR сознательно НЕ делает

- **Не пишет ни одного гварда.** У обоих правил сегодня нет субъекта: `Grant` не существует,
  загруженного содержимого не существует. Гвард против воображаемого случая — это `:data:ai-local` в
  миниатюре, и §3.4 Master Plan сделала из того случая правило блока.
- **Не расширяет словарь типов проверки** — отдельный пункт change-control §5, и повода нет.
- **Не трогает ни одно существующее правило** — ни формулировку, ни тип.
- **Не решает форков A6.** Лестница авторитета модели 0–4 (`improvements.md` Часть 3 F, вторая
  половина) — **не** часть этого правила и сюда не вносится: она меняет не дефолт `localOnlyMode`, а
  его **семантику**, на которой висят дважды поправленный `DOC-ADL-3` и подписанная Class B строка
  `settings_local_only_description`. Это A6.

### Счёт и гейт

Правил в матрице **28 → 30**; `DoctrineMatrixGuardTest.declared_rule_count_matches_the_table` сверяет
объявленное число с фактическим, поэтому расхождение было бы красным, а не незаметным. Префиксы
принципов не менялись (восемь), словарь типов не менялся (шесть). В §3 к перечню адресатов долга
(`A1′`, `A2`, `A4′`) добавлен `A6` — иначе проза перечисляла бы блоки неполно. Журнал §6 получил по
записи на правило.
