...
class AgentVocabularyGuardTest {
    ...
    private val engineRoots = listOf(
        "domain/src/commonMain/kotlin/com/sidr/launcher/domain/agent",
        "domain/src/commonMain/kotlin/com/sidr/launcher/domain/tool",
    ...
    fun `scanned roots all exist and are not empty`() {
        ...
        // nothing — the vacuous-guard shape Этап 2 found in three privacy guards.
     ...
     * never invent a hit — so the guard can in principle be evaded by code no reviewer would write, and
     * can never fail on correct code. A guard that is honest about false negatives beats one that needs a
     * Kotlin parser nobody will maintain.
     */
