...
### Класс fail-open — тронут в одном месте, не как класс
...
- **`DoctrineGuardTest` не пиннит объявленный риск (мутация M9, задача 11).** `SAFE → CONFIRM` на уже
  ...
  nothing», что и `FakeToolRegistry` до этого блока. Задача 12 узко закрывает это парити-тестом —
  **только для двух A0-инструментов**, не для любого инструмента, который зарегистрирует будущая
  федерация.
